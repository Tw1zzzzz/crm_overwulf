...
const StaffManagement: React.FC = () => {
 ...
 // Текущий авторofованный пользователь (для проверки возможности удаления)
 ...
 useEffect(() => {
  // Получаем информацию о текущем пользователе of localStorage
 ...
 // Обработка ofменения ввода в форме
 const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  const { name, value } = e.target;
  setNewStaffData(prev => ({ ...prev, [name]: value }));
  
  // Очищаем ошибку поля при ofменении
  if (formErrors[name]) {
   setFormErrors(prev => {
    const updated = { ...prev };
    delete updated[name];
        ...
        staffList.map(staff => (
          <tr key={getStaffId(staff)} className="hover:bg-gray-50 transition-colors duration-150">
           ...
           <div className="flex justify-center space-x-2">
            {/* Отключаем возможность удалить себя или ofменить свои привилегии */}
