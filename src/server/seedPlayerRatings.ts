...
const seedPlayerRatings = async () => {
 try {
  ...
  console.log(`Найдено ${users.length} players`);
  ...
  console.log(`Добавлено ${playerRatings.length} записей рейтинга players`);
...
// Запуск скрипта
connectDB().then(() => {
 console.log('Запуск заполнения тестовыми данными рейтинга players...');
 seedPlayerRatings();
}); 