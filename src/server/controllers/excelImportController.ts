...
export const importCs2Excel = asyncHandler(async (req: any, res: Response) => {
 ...
 for (let i = 0; i < rowsRaw.length; i++) {
  ...
  if (!userId) {
   rejectedRows++;
   if (errorsSample.length < 50) errorsSample.push({ row: rowIndex, nickname, reason: 'Неofвестный nickname (нет PlayerCard.contacts.nickname)' });
   continue;
  }
  ...
  if (missing.length) {
   rejectedRows++;
   if (errorsSample.length < 50) errorsSample.push({ row: rowIndex, nickname, reason: `No колонок/значений: ${missing.join(', ')}` });
   continue;
  }
