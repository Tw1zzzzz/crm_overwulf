...
// Получение списка players с рейтингом
export const getTopPlayers = async (req: Request, res: Response) => {
 try {
  ...
  // Ищем только players, относящихся к командам
  ...
  // Получаем рейтинги players с информацией о пользователе
  ...
  return res.status(500).json({ 
   message: 'Ошибка при получении рейтинга players',
...
export const updatePlayerRating = async (req: Request, res: Response) => {
 try {
  ...
  if (!userId || !mongoose.Types.ObjectId.isValid(userId)) {
   return res.status(400).json({ message: 'Invalid ID игрока' });
  }
  
  if (!points || isNaN(parseInt(points)) || parseInt(points) <= 0) {
   return res.status(400).json({ message: 'Count очков должно быть положительным числом' });
  }
  ...
  // Находим или создаем запись рейтинга игрока
   ...
   return res.status(400).json({ message: 'Можно ofменять рейтинг только для players в команде' });
  }
