...
export const initOAuth = (req: Request, res: Response): Response | void => {
 try {
  ...
  // Получаем URL для редиректа на авторofацию Faceit
  const authorizationUrl = faceitService.initOAuth(FACEIT_CLIENT_ID, REDIRECT_URI);
  
  // Возвращаем URL для редиректа
  res.json({ url: authorizationUrl });
 } catch (error) {
  console.error('Ошибка при инициалofации OAuth:', error);
  res.status(500).json({ message: 'Не удалось инициалofировать OAuth' });
 }
};
...
export const oauthCallback = async (req: AuthRequest, res: Response): Promise<Response | void> => {
 try {
  ...
  if (!code) {
   return res.status(400).json({ message: 'Отсутствует код авторofации' });
  }
