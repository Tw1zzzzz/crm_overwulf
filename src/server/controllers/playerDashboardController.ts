...
export const getPlayerDashboardByUserId = asyncHandler(async (req: Request, res: Response) => {
 ...
 const drivers = [
  { label: 'Mood', value: moodAggLatest ? avg(moodAggLatest.mood) : null },
  { label: 'Energy', value: moodAggLatest ? avg(moodAggLatest.energy) : null },
  { label: 'Sleep (ч)', value: latestDay ? (sleepByDay.get(latestDay.date) ?? null) : null },
...
export const getPlayerDashboard = async (req: AuthRequest, res: Response) => {
 try {
  ...
  const drivers = [
   { label: 'Average mood (7д)', value: average(moodValues7) },
   { label: 'Wedедняя энергия (7д)', value: average(energyValues7) },
