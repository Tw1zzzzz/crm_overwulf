...
/**
 * Получить корреляции между отчетами команды и настроением players
...
export const getMoodReportsCorrelations = async (req: AuthRequest, res: Response): Promise<Response> => {
 try {
  // Проверка авторofации
  if (!req.user) {
   return res.status(401).json({ 
    success: false, 
    message: 'Не авторofован' 
   });
  }
  ...
  console.log('[CorrelationController] Аналof корреляций настроения и отчетов от', req.user.name);
  ...
  return res.status(500).json({
   success: false,
   message: 'Ошибка при аналofе корреляций настроения и отчетов',
...
/**
 * Получить паттерны проofводительности команды
...
export const getPerformancePatterns = async (req: AuthRequest, res: Response): Promise<Response> => {
 try {
  ...
  console.log('[CorrelationController] Аналof паттернов проofводительности от', req.user.name);
  console.log('[CorrelationController] Period аналofа:', months, 'месяцев');
  ...
  console.error('[CorrelationController] Ошибка получения паттернов проofводительности:', error);
  return res.status(500).json({
   success: false,
   message: 'Ошибка при аналofе паттернов проofводительности',
   error: process.env.NODE_ENV === 'development' ? error.message : undefined
  });
 }
};
...
export const getBalanceWheelReportsCorrelations = async (req: AuthRequest, res: Response): Promise<Response> => {
 try {
  ...
  console.log('[CorrelationController] Аналof корреляций колеса баланса от', req.user.name);
  ...
  return res.status(500).json({
   success: false,
   message: 'Ошибка при аналofе корреляций колеса баланса и отчетов',
   error: process.env.NODE_ENV === 'development' ? error.message : undefined
  });
 }
};
...
/**
 * Получить комплексный корреляционный аналof
...
export const getComprehensiveAnalysis = async (req: AuthRequest, res: Response): Promise<Response> => {
 try {
  ...
  console.log('[CorrelationController] Comprehensive корреляционный аналof от', req.user.name);
  console.log('[CorrelationController] Period:', { dateFrom, dateTo });
  ...
  console.error('[CorrelationController] Ошибка комплексного аналofа:', error);
  return res.status(500).json({
   success: false,
   message: 'Ошибка при выполнении комплексного корреляционного аналofа',
   error: process.env.NODE_ENV === 'development' ? error.message : undefined
  });
 }
};
...
/**
 * Получить статистику корреляционного аналofа
...
export const getCorrelationStats = async (req: AuthRequest, res: Response): Promise<Response> => {
 try {
  ...
  // Быстрый аналof для получения общей статистики
  const [moodCorrelations, patterns] = await Promise.all([
   analyzeReportMoodCorrelations(),
   analyzeTeamPerformancePatterns(3) // Последние 3 months для быстрой статистики
  ]);
...
/**
 * Сгенерировать AI-вывод по данным корреляционного аналofа
...
export const getCorrelationAssistantInsight = async (req: AuthRequest, res: Response): Promise<Response> => {
 try {
  ...
  if (!payload || !Array.isArray(payload.metricSummaries) || payload.metricSummaries.length === 0) {
   return res.status(400).json({
    success: false,
    message: 'Недостаточно данных для AI-аналofа'
   });
  }
  ...
  console.error('[CorrelationController] Ошибка AI-аналofа корреляций:', error);
  return res.status(500).json({
   success: false,
   message: 'Ошибка при генерации AI-аналofа',
   error: process.env.NODE_ENV === 'development' ? error.message : undefined
  });
 }
};
...
/**
 * Получить мультиметричные данные для корреляционного аналofа
 */
export const getMultiMetrics = async (req: AuthRequest, res: Response) => {
 try {
  ...
  // Получаем данные of всех коллекций параллельно
