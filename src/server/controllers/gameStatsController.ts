...
export const createGameStats = async (req: AuthRequest, res: Response) => {
 try {
  ...
  if (ctSide.pistolRoundsWon > ctSide.pistolRounds) {
   return res.status(400).json({ 
    message: 'CT Side: Count выигранных пистолетных раундов не может превышать общее количество пистолетных раундов' 
   });
  }
  ...
  if (tSide.pistolRoundsWon > tSide.pistolRounds) {
   return res.status(400).json({ 
    message: 'T Side: Count выигранных пистолетных раундов не может превышать общее количество пистолетных раундов' 
   });
  }
...
/**
 * Получение топ players по игровым показателям
 */
export const getTopPlayersByGameStats = async (req: Request, res: Response) => {
 try {
  ...
  console.error('Ошибка при получении топ players:', error);
  res.status(500).json({ message: 'Внутренняя ошибка сервера' });
 }
};
