...
// Настройка хранения ofображений
...
const fileFilter = (req: Request, file: Express.Multer.File, cb: multer.FileFilterCallback) => {
 ...
 if (allowedTypes.includes(file.mimetype)) {
  ...
  cb(new Error('Разрешены только ofображения форматов: .jpg, .jpeg, .png, .webp'));
 }
};

// Инициалofация multer
...
export const getPlayerCard = async (req: Request, res: Response) => {
 try {
  ...
  if (!userId) {
   return res.status(400).json({ success: false, message: 'ID пользователя not specified' });
  }
  ...
  // ofменено с автоматического создания на явный запрос создания карточки
  if (!playerCard) {
   return res.status(404).json({ success: false, message: 'Card игрока не найдена' });
  }

  // Получаем данные о проofводительности игрока of других коллекций, если доступны
...
export const getAllPlayerCards = async (req: Request, res: Response) => {
 try {
  ...
  if (!playerCards || playerCards.length === 0) {
   return res.status(200).json({
    success: true,
    message: 'Карточки players не найдены',
    data: [],
    pagination: {
     total,
     page,
  ...
  console.error('Ошибка при получении списка карточек players:', error);
  return res.status(500).json({ 
   success: false, 
   message: 'Ошибка сервера при получении списка карточек players' 
  });
 }
};
...
export const createPlayerCard = async (req: Request, res: Response) => {
 try {
  ...
  if (!userId) {
   console.error('Ошибка: ID пользователя not specified');
   return res.status(400).json({
    success: false,
    message: 'ID пользователя not specified'
   });
  }
  ...
  if (existingCard) {
   console.error(`Ошибка: Card игрока уже существует для пользователя: ${userId}`);
   return res.status(400).json({ 
    success: false, 
    message: 'Card игрока уже существует для этого пользователя' 
   });
  }

  // Извлекаем и валидируем данные of запроса
  const { 
   vk = '', 
   telegram = '', 
   faceit = '', 
  ...
  console.log(`Card игрока для пользователя ${userId} успешно создана:`, {
   id: playerCard._id,
  ...
  return res.status(201).json({
   success: true,
   message: 'Card игрока успешно создана',
   playerCard,
   user: {
    id: user._id.toString(),
    name: user.name,
...
export const updateContacts = async (req: Request, res: Response) => {
 try {
  ...
  if (!contacts) {
   return res.status(400).json({ 
    success: false, 
    message: 'Data контактов не предоставлены' 
   });
  }
  ...
  if (!existingCard) {
   return res.status(404).json({ 
    success: false, 
    message: 'Card игрока не найдена' 
   });
  }
  ...
  return res.status(200).json({ 
   success: true,
   message: 'Contacts успешно обновлены', 
   playerCard 
  });
 } catch (error) {
...
/**
 * Upload ofображение для коммуникативной линии
 * @param req - запрос
 * @param res - ответ
 */
export const uploadCommunicationImage = async (req: Request, res: Response) => {
 try {
  ...
  console.log(`Сохраняем путь к ofображению коммуникативной линии в БД: ${playerCard.communicationImage}`);
  ...
  return res.status(200).json({ 
   success: true,
   message: 'Communication line image успешно загружено', 
   communicationImage: playerCard.communicationImage 
  });
 } catch (error) {
  console.error('Ошибка при загрузке ofображения коммуникативной линии:', error);
  return res.status(500).json({ 
   success: false,
   message: 'Ошибка сервера при загрузке ofображения коммуникативной линии',
   error: error instanceof Error ? error.message : 'Неofвестная ошибка'
  });
 }
};
...
export const deletePlayerCard = async (req: Request, res: Response) => {
 try {
  ...
  // Удаляем карточку of базы данных
  const deleteResult = await PlayerCard.deleteOne({ userId });
  
  if (deleteResult.deletedCount === 0) {
   return res.status(404).json({ 
    success: false, 
    message: 'Card игрока не найдена или уже удалена' 
   });
  }

  // Удаляем связанные файлы после успешного удаления of БД
  ...
  return res.status(200).json({ 
   success: true,
   message: 'Card игрока успешно удалена',
...
/**
 * Link player к существующей карточке
 * @param req - запрос
 * @param res - ответ
 */
export const attachPlayerToCard = async (req: Request, res: Response) => {
 try {
  ...
  if (!existingCard) {
   return res.status(404).json({ 
    success: false, 
    message: 'Card с указанным ID не найдена' 
   });
  }
  ...
  if (!(await canAccessCard(req, existingCard.userId.toString()))) {
   return res.status(403).json({
    success: false,
    message: 'No доступа к этой карточке игрока'
   });
  }
  ...
  console.log(`Card ${cardId} успешно привязана к игроку ${newUserId} (ранее была привязана к ${oldUserId})`);
