...
/**
 * Получение аналofа сентимента отчетов команды
 */
export const getSentimentAnalysis = async (req: Request, res: Response) => {
 try {
  ...
  console.log('[AdvancedAnalytics] Получение аналofа сентимента, период:', fromDate, '->', toDate);
  ...
  console.error('[AdvancedAnalytics] Ошибка аналofа сентимента:', error);
  res.status(500).json({
   success: false,
   message: 'Ошибка при аналofе сентимента отчетов',
...
/**
 * Получение кластерного аналofа players
 */
export const getPlayerClustering = async (req: Request, res: Response) => {
 try {
  console.log('[AdvancedAnalytics] Выполнение кластерного аналofа players');
  ...
  console.error('[AdvancedAnalytics] Ошибка кластерного аналofа:', error);
  res.status(500).json({
   success: false,
   message: 'Ошибка при выполнении кластерного аналofа players',
   error: process.env.NODE_ENV === 'development' ? error.message : undefined
  });
 }
};
...
/**
 * Получение аналofа временных рядов
 */
 ...
 try {
  ...
  if (daysBack) {
   ...
   if (isNaN(days) || days < 7 || days > 365) {
    return res.status(400).json({
     success: false,
     message: 'Count дней должно быть от 7 до 365'
    });
   }
  }

  console.log('[AdvancedAnalytics] Аналof временных рядов:', metric, 'за', days, 'дней');
  ...
  console.error('[AdvancedAnalytics] Ошибка аналofа временных рядов:', error);
  res.status(500).json({
   success: false,
   message: 'Ошибка при аналofе временных рядов',
   error: process.env.NODE_ENV === 'development' ? error.message : undefined
  });
 }
};
...
/**
 * Получение профиля проofводительности команды
 */
export const getTeamPerformanceProfile = async (req: Request, res: Response) => {
 try {
  console.log('[AdvancedAnalytics] Создание профиля проofводительности команды');
  ...
  res.status(500).json({
   success: false,
   message: 'Ошибка при создании профиля проofводительности команды',
   error: process.env.NODE_ENV === 'development' ? error.message : undefined
  });
 }
};
