...
export const createReport = async (req: any, res: any) => {
 try {
  ...
  try {
   if (files.length > 0) {
    // Перемещаем файлы of temp в постоянную директорию
...
export const updateReportStatus = async (req: any, res: any) => {
 try {
  ...
  if (!report.canBeEditedBy(userId)) {
   return res.status(403).json({
    success: false,
    message: 'Недостаточно прав для ofменения статуса этого отчета'
   });
  }
  ...
  res.status(200).json({
   success: true,
   message: `Статус отчета ofменен на "${status}"`,
   data: report
  });
  
 } catch (error) {
  console.error('[TeamReports Controller] Ошибка при ofменении статуса:', error);
  res.status(500).json({
   success: false,
   message: 'Ошибка при ofменении статуса отчета',
   error: error instanceof Error ? error.message : 'Неofвестная ошибка'
  });
 }
};
