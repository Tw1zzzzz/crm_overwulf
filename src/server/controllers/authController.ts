...
const ensureTeamPlayerCapacity = async (teamId: mongoose.Types.ObjectId): Promise<void> => {
 ...
 if (playersCount >= team.playerLimit) {
  throw new Error('Лимит players в этой команде уже достигнут');
 }
};
...
export const registerUser = async (req: any, res: any) => {
 try {
  ...
  if (!name || !email || !password) {
   console.log('[AuthController] Ошибка: missing обязательные поля');
  ...
  if (mongoose.connection.readyState !== 1) {
   return res.status(503).json({
    message: 'База данных инициалofируется, повторите регистрацию через несколько секунд'
   });
  }
  ...
  if (teamName) {
   return res.status(403).json({
    message: 'Создание команды доступно только авторofованному сотруднику через раздел управления командами'
   });
  }
  ...
  try {
   ...
   return res.status(500).json({ 
    ...
    error: findError instanceof Error ? findError.message : 'Неofвестная ошибка'
   });
  }
  ...
  try {
   ...
   try {
    ...
    return res.status(500).json({
     ...
     error: mongoError.message || 'Неofвестная ошибка MongoDB'
    });
   }
   ...
   if (user) {
    if (finalRole === 'player') {
     try {
      ...
      console.error('[AuthController] Ошибка инициалofации данных игрока после регистрации:', setupError);
      return res.status(500).json({
       message: 'Пользователь создан, но не удалось инициалofировать данные игрока',
       error: setupError instanceof Error ? setupError.message : 'Неofвестная ошибка'
      });
     }
    }
    ...
    res.status(201).json({
     message: emailDeliveryFailed
      ...
      : 'Аккаунт создан. Подтвердите email по ссылке of письма, затем войдите в систему.',
   ...
   return res.status(500).json({ 
    message: 'Ошибка при создании пользователя',
    error: createError instanceof Error ? createError.message : 'Неofвестная ошибка'
   });
  }
 } catch (error) {
  ...
  return res.status(500).json({ 
   ...
   error: error instanceof Error ? error.message : 'Неofвестная ошибка'
  });
 }
};
...
export const loginWithOverwolf = async (req: any, res: any) => {
 try {
  ...
  if (mongoose.connection.readyState !== 1) {
   return res.status(503).json({
    message: 'База данных инициалofируется, повторите вход через несколько секунд'
   });
  }
...
export const changePassword = async (req: any, res: any) => {
 try {
  if (!req.user?._id) {
   return res.status(401).json({ message: 'Пользователь не авторofован' });
  }
  ...
  return res.json({ message: 'Пароль успешно ofменен. Выполните вход повторно.' });
 } catch (error) {
...
export const updateCurrentUserProfile = async (req: any, res: any) => {
 try {
  if (!req.user?._id) {
   return res.status(401).json({ success: false, message: 'Пользователь не авторofован' });
  }
  ...
  if (req.body?.name !== undefined) {
   if (typeof req.body.name !== 'string') {
    return res.status(400).json({ success: false, message: 'Name должно быть строкой' });
   }

   const trimmedName = req.body.name.trim();
   if (!trimmedName) {
    return res.status(400).json({ success: false, message: 'Name не может быть пустым' });
   }
  ...
  if (shouldUpdateFaceit) {
   ...
   try {
    ...
    return res.status(400).json({
     success: false,
     message: `Не удалось получить FACEIT профиль: ${error instanceof Error ? error.message : 'Неofвестная ошибка'}`
    });
   }
  ...
  if (!Object.keys(updates).length && !shouldUpdateFaceit) {
   return res.status(400).json({ success: false, message: 'No данных для обновления профиля' });
  }
...
export const linkTeamProfile = async (req: any, res: any) => {
 try {
  ...
  if (activeProfile.role === 'staff' && resolvedTeamAccess.invitedRole !== 'staff') {
   return res.status(403).json({
    message: 'Стафф может привязаться к команде только по staff codeу'
   });
  }
  ...
  if (existingTeamProfile?.teamId && sameObjectId(existingTeamProfile.teamId, resolvedTeamAccess.team._id)) {
   ...
   return res.json({
    status: 'linked',
    message: 'Team profile уже привязан к этой команде',
    targetProfileKey,
    team: nextTeam,
    user: buildUserResponse(updatedUser, accessFlags)
   });
  ...
  if (currentTeam?.id && !sameObjectId(currentTeam.id, resolvedTeamAccess.team._id) && !confirmRelink) {
   return res.json({
    ...
    message: `Team profile уже привязан к команде «${currentTeam.name || 'Без названия'}». Подтвердите перепривязку к «${nextTeam.name}».`,
    targetProfileKey,
    currentTeam,
    nextTeam
   });
  ...
  return res.json({
   ...
   message:
    currentTeam?.id && !sameObjectId(currentTeam.id, resolvedTeamAccess.team._id)
     ? 'Team profile успешно перепривязан к новой команде'
     : 'Team profile успешно привязан к команде',
   targetProfileKey,
   team: nextTeam,
   user: buildUserResponse(updatedUser, accessFlags)
  });
  ...
  if (error instanceof Error && error.message === 'Лимит players в этой команде уже достигнут') {
...
export const getCurrentUser = async (req: any, res: any) => {
 try {
  ...
  // Получаем актуальную информацию о пользователе of базы данных
  ...
  console.log(`[AuthController] Role: ${user.role}`);
