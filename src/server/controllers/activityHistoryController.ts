...
export const getAllActivityHistory = async (req: AuthRequest, res: Response): Promise<Response | void> => {
 try {
  ...
  console.log(`[ActivityHistory] Sunего найдено записей: ${total}`);
  ...
  if (activities.length > 0) {
   console.log('[ActivityHistory] Пример записи активности:', {
    ...
    user: typeof activities[0].userId === 'object' ? 
     ((activities[0].userId as any)?.name || 'Неofвестный пользователь') : 
...
export const getMonthlyActivity = async (req: AuthRequest, res: Response): Promise<Response | void> => {
 try {
  ...
  try {
   ...
   console.log(`[ActivityMonthly] Sunего записей активности в базе: ${totalActivities}`);
   ...
   // Получаем players для информации
   const players = await User.find({ role: { $in: ['user', 'player'] } }, '_id name email role');
   console.log(`[ActivityMonthly] Найдено ${players.length} players`);
    ...
    for (let i = 0; i < Math.min(3, activities.length); i++) {
     ...
     console.log(`[ActivityMonthly] Запись ${i+1}:`, {
      ...
      userId: typeof activity.userId === 'object' 
       ? `${(activity.userId as any)?.name || 'No имени'} (ID: ${(activity.userId as any)?._id || 'No ID'})` 
       : `ID: ${String(activity.userId)}`,
      details: activity.details || 'No деталей'
     });
    }
    ...
    if (moodActivities.length > 0) {
     console.log('[ActivityMonthly] Пример записи о настроении:', {
      ...
      userId: typeof moodActivities[0].userId === 'object' 
       ? `${(moodActivities[0].userId as any)?.name || 'No имени'} (ID: ${(moodActivities[0].userId as any)?._id || 'No ID'})` 
       : `ID: ${String(moodActivities[0].userId)}`,
      details: moodActivities[0].details || 'No деталей'
     });
    }
   }
   ...
   if (playerActivities.length > 0) {
    ...
    console.log('[ActivityMonthly] No активности players, возвращаем демо-данные');
    return res.status(200).json({
     status: 'success',
     data: {
   ...
   return res.status(200).json({
    status: 'success',
    message: 'Используются демонстрационные данные of-за ошибки',
    data: {
     activities: generateDemoActivities(req.user._id),
     period: {
      start: startOfMonth.toISOString(),
...
function generateDemoActivities(userId: string | any) {
 ...
 // Создаем тестовые активности за последние 30 days
 for (let i = 0; i < 15; i++) {
  const date = new Date();
