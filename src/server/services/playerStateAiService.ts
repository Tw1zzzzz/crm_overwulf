...
const buildFallbackAnalysis = (data: AggregatedPlayerData): PlayerStateReport => {
 ...
 if (reactionKeys.length > 0) headBreakdown.push(`Reaction: ${Math.round(avg(reactionKeys.map((k) => data.brainDomainScores[k])))}`);
 ...
 if (avgScreenTime > 0) eyesBreakdown.push(`Экранное время: ${avgScreenTime.toFixed(1)} h/day`);
 if (avgEntertainment > 0) eyesBreakdown.push(`Entertainment: ${avgEntertainment.toFixed(1)} h/day`);
 ...
 if (avgMood > 0) chestBreakdown.push(`Mood: ${avgMood.toFixed(1)}/10`);
 if (snapStress > 0) chestBreakdown.push(`Stress: ${snapStress.toFixed(1)}/10`);
 if (bw) chestBreakdown.push(`Emotions (колесо): ${Math.round((bw.emotional / 10) * 100)}`);
 ...
 if (gs) {
  ...
  armsBreakdown.push(`Matchей: ${gs.totalMatches}`);
 } else {
  armsBreakdown.push('Игровые данные missing');
 }

 const backBreakdown: string[] = [];
 if (snapFatigue > 0) backBreakdown.push(`Fatigue: ${snapFatigue.toFixed(1)}/10`);
 if (snapSleep > 0) backBreakdown.push(`Sleep: ${snapSleep.toFixed(1)} ч`);
 if (avgEnergy > 0) backBreakdown.push(`Energy: ${avgEnergy.toFixed(1)}/10`);
 if (bw) backBreakdown.push(`Фof. (колесо): ${Math.round((bw.physical / 10) * 100)}`);
 ...
 if (bw) {
  ...
  legsBreakdown.push('Data колеса баланса missing');
 }
 ...
 const zones: Record<BodyZone, ZoneData> = {
  head: {
   ...
   description: `Cognitive потенциал: балл ${headScore}/100. ${headScore >= 75 ? 'Когнитивные ресурсы в норме.' : headScore >= 50 ? 'Возможно небольшое снижение концентрации.' : 'Высокая когнитивная нагрузка, рекомендуется отдых.'}`,
  ...
  eyes: {
   ...
   description: `Focus и концентрация: балл ${eyesScore}/100. ${eyesScore >= 75 ? 'Load на зрение в пределах нормы.' : eyesScore >= 50 ? 'Умеренная нагрузка на зрение.' : 'Высокое экранное время, зрение перегружено.'}`,
  ...
  chest: {
   ...
   description: `Psychological core: балл ${chestScore}/100. ${chestScore >= 75 ? 'Эмоциональный фон стабильный.' : chestScore >= 50 ? 'Умеренный уровень стресса.' : 'Высокий стресс, требуется восстановление.'}`,
  ...
  arms: {
   ...
   description: `Game performance: балл ${armsScore}/100. ${gs ? `K/D: ${gs.avgKdRatio.toFixed(2)}, Win Rate: ${gs.avgWinRate.toFixed(1)}%.` : 'Игровые данные за последние 30 days missing.'}`,
  ...
  legs: {
   ...
   description: `Жofненный фундамент: балл ${legsScore}/100. ${legsScore >= 75 ? 'Жofненный баланс хороший.' : legsScore >= 50 ? 'Умеренный уровень жofненного баланса.' : 'Нofкий уровень жofненного баланса.'}`,
 ...
 if (data.moodEntries.length > 0) dataUsed.push(`Mood and energy: ${data.moodEntries.length} записей`);
 ...
 if (data.gameStatsSummary) dataUsed.push(`Game statistics: ${data.gameStatsSummary.totalMatches} матчей`);
 ...
 const recommendations = [
  ...
  `Уделите особое внимание зоне "${worstZone.zone === 'head' ? 'Cognitive потенциал' : worstZone.zone === 'eyes' ? 'Focus и концентрация' : worstZone.zone === 'chest' ? 'Psychological core' : worstZone.zone === 'arms' ? 'Game performance' : worstZone.zone === 'back' ? 'Восстановление' : 'Жofненный фундамент'}" с баллом ${worstZone.score}/100.`,
 ];

 return {
  report: `Аналof состояния игрока сформирован на основе доступных данных. Wedедний балл по всем зонам: ${Math.round(avgZoneScore)}/100.\n\nMood and energy отражают ${avgMood > 0 ? `средний уровень ${avgMood.toFixed(1)}/10 по настроению и ${avgEnergy.toFixed(1)}/10 по энергии за последние 14 дней` : 'недостаточно данных для точной оценки'}.\n\nЭмоциональный фон ${chestScore >= 75 ? 'стабилен' : chestScore >= 50 ? 'умеренно напряжён' : 'требует внимания'}, уровень стресса ${zones.chest.label.toLowerCase()}.\n\nФofическая готовность и энергия оцениваются как "${zones.legs.label}", игровая проofводительность "${zones.arms.label}".\n\nАналof выполнен автоматически без участия AI в связи с недоступностью API.`,
  zones,
  recommendations,
  dataUsed,
...
Аналofируй данные объективно: не придумывай причин, если они не следуют of цифр.

Рассматривай игрока как целостную систему: нейро-когнитивные способности, психологическое состояние, образ жofни и игровые показатели взаимосвязаны. Объясни, как внеигровые факторы (сон, стресс, психология, когниции) влияют на игровую результативность.

Оцени шесть зон по шкале 0–100:
- head (Cognitive потенциал): результаты brain-тестов (домены attention, working_memory, reaction, flexibility) + intellectual of колеса баланса. Отражает способность к обучению, скорость принятия решений, рабочую память.
- eyes (Focus и концентрация): go/no-go тест (reaction_inhibition), stroop (flexibility), экранное время (totalTime, entertainment hours). Отражает устойчивость фокуса и селективное внимание.
- chest (Psychological core): настроение (mood), уровень стресса (stress snapshot), emotional of колеса баланса. Отражает эмоциональную стабильность, устойчивость к давлению, мотивацию.
- arms (Game performance): K/D ratio, Win Rate, ADR (если есть) of game stats, количество матчей. Отражает итоговую результативность в игре.
- back (Восстановление): fatigue snapshot, sleep hours, energy of mood entries, physical of колеса баланса. Отражает способность восстанавливаться и поддерживать форму.
- legs (Жofненный фундамент): social, environmental, financial, occupational of колеса баланса. Отражает стабильность жofненного контекста как основы для роста.
...
Для каждой зоны сформируй dataBreakdown — массив of 2–5 строк с конкретными числовыми показателями, питающими зону (напр. "Внимание: 78", "K/D: 1.42", "Stress: 6.2/10").

Верни JSON-объект строго в формате:
\`\`\`json
{
 "report": "3-5 абзацев текстового аналofа на русском, объясняющего взаимосвязи между зонами",
 "zones": {
...
export const generatePlayerStateReport = async (
 ...
 if (!apiKey) {
  console.warn('[PlayerStateAI] OPENROUTER_API_KEY не задан, используется fallback аналof');
