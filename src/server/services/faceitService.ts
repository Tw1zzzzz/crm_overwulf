...
const hasFaceitApiKey = (): boolean => {
 ...
 const placeholders = ['ваш_клюh_faceit_api', 'YOUR_FACEIT_API_KEY'];
...
/**
 * Инициалofирует процесс OAuth авторofации
 * @param clientId - ID клиента Faceit
 * @param redirectUri - URI для перенаправления после авторofации
...
/**
 * Обменивает код авторofации на токены доступа
 * @param code - Код авторofации от Faceit
 * @param clientId - ID клиента Faceit
 ...
 * @returns Data пользователя Faceit
 */
 ...
 * @param accessToken - Токен доступа
 * @param refreshToken - Refresh токен
 * @param expiresIn - Время жofни токена в секундах
...
export const saveFaceitAccount = async (
 ...
 try {
  ...
  // Гарантируем синхронofацию связи в профиле пользователя
  await User.findByIdAndUpdate(userId, { faceitAccountId: faceitAccount._id });
...
export const importMatches = async (faceitAccountId: string | Types.ObjectId): Promise<number> => {
 try {
  ...
  for (const matchData of matches) {
   ...
   // elo_before/elo_after missing в Open Data API истории — оставляем 0.
