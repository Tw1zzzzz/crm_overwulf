...
export const sendPasswordResetEmail = async ({
 ...
 const { error } = await resend.emails.send({
  ...
  to: email,
  subject: 'Password reset в Performance CRM',
  text: [
   `Здравствуйте, ${greetingName}!`,
   '',
   'Мы получили запрос на смену пароля в Performance CRM.',
   'Thuобы задать новый пароль, перейдите по ссылке ниже:',
   resetUrl,
   '',
  ...
  html: renderEmailLayout({
   ...
   intro: 'Мы получили запрос на смену пароля в Performance CRM. Thuобы задать новый пароль, перейдите по кнопке ниже.',
...
export const sendSupportRequestEmail = async ({
 ...
 const resend = new Resend(process.env.RESEND_API_KEY);
 const safeName = name?.trim() || 'Not specified';
 const safeCategory = supportCategoryLabels[category.trim()] || category.trim();
 const safeSubject = subject.trim();
 const safeMessage = message.trim();
 const safePageUrl = pageUrl?.trim() || 'Not specified';
 const safeUserAgent = userAgent?.trim() || 'Not specified';
    ...
    <div style="max-width:640px;margin:0 auto;background:#ffffff;border:1px solid #e5e7eb;border-radius:20px;overflow:hidden;box-shadow:0 12px 40px rgba(15,23,42,0.08);">
     ...
     <div style="padding:28px 32px;">
      <table style="width:100%;border-collapse:collapse;">
       <tbody>
        <tr>
         <td style="padding:10px 0;border-bottom:1px solid #e5e7eb;color:#6b7280;font-size:14px;font-weight:700;width:160px;">Name</td>
        ...
        <tr>
         <td style="padding:10px 0;border-bottom:1px solid #e5e7eb;color:#6b7280;font-size:14px;font-weight:700;">Category</td>
        ...
        <tr>
         <td style="padding:10px 0;border-bottom:1px solid #e5e7eb;color:#6b7280;font-size:14px;font-weight:700;">Page</td>
