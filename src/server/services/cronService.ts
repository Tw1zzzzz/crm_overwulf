...
class CronService {
 ...
 /**
  * Удаляет задачу of планировщика
  * @param name - Name задачи для удаления
 ...
 removeTask(name: string): boolean {
  ...
  if (task) {
   ...
   console.log(`Задача "${name}" успешно удалена of расписания`);
   return true;
  }
  return false;
 }
 ...
 /**
  * Останавливает выполнение задачи (без удаления of списка)
  * @param name - Name задачи для остановки
  ...
  * @param name - Name задачи для возобновления
