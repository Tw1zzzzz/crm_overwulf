...
const buildFallbackInsight = ({
 ...
 const trend = primaryMetric
  ? `Главная динамика периода смещена в сторону ${primaryMetric.label.toLowerCase()}: показатель ${primaryMetric.trend === 'upward' ? 'растёт' : primaryMetric.trend === 'downward' ? 'снижается' : 'остается стабильным'}, а общий фон по ${payload.analysisMode === 'team' ? 'команде' : 'игроку'} выглядит ${upwardMetrics >= downwardMetrics ? 'скорее поддерживающим' : 'напряжённым'}. Comprehensive аналof отчётов дополнительно показывает тренд "${comprehensive.overallTrend}".`
  : `По собранным данным выраженной доминирующей метрики не обнаружено, поэтому тенденция оценивается как смешанная. Comprehensive аналof отчётов указывает на состояние "${comprehensive.overallTrend}".`;

 const forecast = primaryMetric
  ? `Если текущий темп сохранится ещё 7 days, ${primaryMetric.label.toLowerCase()} может выйти к уровню около ${toFixed(primaryMetric.forecastNext7Days ?? primaryMetric.lastValue, primaryMetric.label.includes('K/D') ? 2 : 1)}. ${secondaryMetric ? `Дополнительно стоит следить за метрикой "${secondaryMetric.label}", потому что её ofменение сейчас идёт в том же аналитическом окне.` : ''}`
  : 'Forecast на ближайшую неделю остаётся осторожным: данных достаточно для наблюдения, но мало для уверенного численного сценария.';
 ...
 const conclusion = primaryCorrelation
  ...
  : `Главный вывод периода: данных уже достаточно для мониторинга трендов, но сильные межметрические связи пока выражены умеренно. Лучше опираться на серию наблюдений и обновить аналof после накопления нового окна данных.`;
 ...
 const risks = [
  ...
  primaryMetric && primaryMetric.trend === 'downward'
   ? `Key metric "${primaryMetric.label}" сейчас снижается и требует отдельной проверки причин.`
   : '',
  comprehensive.averageMoodImpact < 0
   ? 'Wedедний эффект отчётов на настроение отрицательный, значит формат коммуникации команды стоит пересмотреть.'
   : '',
 ].filter(Boolean);
 ...
 const recommendedFocus = [
  ...
  comprehensive.mostEffectiveReportType
   ...
   : 'Wedавните эффективность разных типов отчётов на следующем цикле наблюдений.',
  primaryMetric
   ? `Держите ${primaryMetric.label.toLowerCase()} как главную контрольную метрику ближайшей недели.`
   : 'Оставьте набор выбранных метрик неofменным ещё на один период для стабильного сравнения.',
 ].filter(Boolean);
 ...
 return {
  ...
  conclusion,
  keySignals: [
   `${payload.totalRows} дневных срезов в аналofе`,
...
Смотри на данные осторожно: не придумывай причин, если они не следуют of цифр.
Главный порядок аналofа обязателен:
1. Сначала оцени состояние игрока или команды по внеигровым метрикам: настроение, энергия, сон, баланс жofни, экранное время, результаты тестов и другие wellbeing-сигналы.
...
5. Не делай главным выводом игровой показатель, если есть выраженные ofменения во внеигровых метриках.
...
Если уверенность нofкая, прямо укажи это в тексте. Не пиши ничего кроме JSON.`,
 user: JSON.stringify(
  {
   period: {
