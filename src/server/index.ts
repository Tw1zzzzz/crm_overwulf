...
// Загружаем .env файл of корня проекта
const envPath = path.resolve(__dirname, '../../.env');
console.log('[INDEX] Загрузка .env of:', envPath);
...
// Импортируем app of server.ts ПОСЛЕ загрузки env
...
// Определяем порт (SERVER_PORT имеет приоритет, чтобы ofбежать конфликта с Vite)
...
const startServer = async () => {
 try {
  ...
  const server = app.listen(listenPort, HOST, () => {
   ...
   // Инициалofация задач синхронofации с Faceit
