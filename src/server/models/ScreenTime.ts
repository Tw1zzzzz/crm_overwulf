...
export interface IScreenTime extends Document {
 ...
 date: Date;
 totalTime: number; // Total time за экраном
 entertainment: number; // Entertainment (игры, видео, фильмы)
 communication: number; // Communication (соцсети, мессенджеры, звонки)
 browser: number; // Браузер (серфинг, новости, покупки)
 study: number; // Study (курсы, обучающие материалы, работа)
