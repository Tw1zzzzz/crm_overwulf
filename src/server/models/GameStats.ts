...
export interface IGameStats extends Document {
 ...
 // Overall statistics (суммарная по обеим сторонам)
 totalMatches: number;
 wins: number;
 losses: number;
 draws: number;
 ...
 // Statistics по сторонам
 ctSide: SideStats;
 tSide: SideStats;
 
 // Overall statistics раундов (сумма CT + T)
 ...
 // Overall statistics пистолетных раундов
...
GameStatsSchema.pre('save', function(next) {
 ...
 // Автозаполнение проofводных метрик, если аналитик не ввел их вручную
 if (this.kpr == null) {
...
// Создание индексов для оптимofации запросов
