...
const userSchema = new mongoose.Schema(
 {
  name: {
   type: String,
   required: [true, "Name обязательно"],
   trim: true,
  },
  email: {
   type: String,
...
userSchema.pre("save", async function (next) {
 try {
  ...
  // Проверяем, существует ли this и был ли пароль ofменен
  if (!this.isModified("password")) {
   console.log('[User Model] Пароль не был ofменен, пропускаем хеширование');
   return next();
  }
  ...
  if (!this.password) {
   ...
   return next(new Error('Пароль not specified'));
  }
...
// Method для сравнения паролей
userSchema.methods.matchPassword = async function (enteredPassword: string): Promise<boolean> {
 try {
  console.log('[User Model] Wedавнение паролей');
