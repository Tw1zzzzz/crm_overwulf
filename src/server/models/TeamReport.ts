...
export enum ReportVisibility {
 PUBLIC = 'public',   // Sunе игроки видят
...
const teamReportSchema = new mongoose.Schema<TeamReportDocument>({
 title: {
  type: String,
  required: [true, 'Name отчета обязательно'],
  trim: true,
  minlength: [3, 'Name должно содержать минимум 3 символа'],
  maxlength: [200, 'Name не может превышать 200 символов'],
  index: true
 },
 ...
 content: {
  ...
  validate: {
   ...
   message: 'Report должен содержать хотя бы одну секцию'
  }
 },
 ...
 type: {
  ...
  required: [true, 'Type отчета обязателен'],
  index: true
 },
 ...
 // Настройки для JSON сериалofации
 toJSON: {
  virtuals: true,
...
/**
 * Indexы для оптимofации запросов
 */
// Rosterной индекс для поиска по статусу и видимости
teamReportSchema.index({ status: 1, visibility: 1 });

// Rosterной индекс для поиска по создателю и дате
teamReportSchema.index({ createdBy: 1, createdAt: -1 });

// Rosterной индекс для поиска по типу и дате
...
teamReportSchema.pre('save', function(next) {
 ...
 // Увеличиваем версию при ofменении содержимого
