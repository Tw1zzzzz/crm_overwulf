...
export const BASELINE_QUESTIONS: BaselineQuestion[] = [
 ...
 },
 {
  ...
  options: [
   {
    id: 'shift_force',
    label: 'Wedазу перестраиваюсь через агрессию и новый темп.',
    weights: { tempo: 3, pressureResponse: 1 }
   },
   {
    id: 'shift_sync',
   ...
   {
    id: 'shift_compose',
    label: 'Сначала стабилofируюсь, потом действую.',
    weights: { pressureResponse: 3, decisionStyle: 1 }
   }
  ]
 },
 {
  ...
  prompt: 'Thuо у вас чаще всего усиливается в клатч-ситуации?',
  options: [
   {
   ...
   {
    id: 'pressure_read',
    label: 'Thuение оппонента и расчёт вероятностей.',
    weights: { decisionStyle: 3, pressureResponse: 1 }
   },
   {
    id: 'pressure_calm',
...
> = {
 ...
 'Командный стабилofатор': {
  headline: 'Вы усиливаете команду через связь, собранность и ощущение общего плана.',
  description:
   'Ваш стиль особенно ценен там, где нужно удержать коммуникацию чистой, вовремя синхронofировать людей и вернуть структуру в хаос.',
...
const ARCHETYPE_BY_AXIS: Record<BaselineAxis, string> = {
 tempo: 'Драйвер темпа',
 communication: 'Командный стабилofатор',
...
export function validateBaselineAnswers(answers: BaselineAnswerInput[]) {
 ...
 for (const answer of answers) {
  ...
  if (!question) {
   return { valid: false, message: `Неofвестный вопрос: ${answer.questionId}` };
  }

  if (uniqueQuestionIds.has(answer.questionId)) {
   return { valid: false, message: 'Один of вопросов заполнен несколько раз.' };
  }
  ...
  if (!option) {
   return {
    valid: false,
    message: `Для вопроса ${answer.questionId} выбран неofвестный вариант ответа.`
   };
  }
 }
