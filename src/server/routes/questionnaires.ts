 ...
 asyncHandler(async (req: any, res) => {
  ...
  if (resolvedSleepHours != null) {
   const sleepComment = sleepStartTime && sleepEndTime
    ? `Sleep: с ${sleepStartTime} до ${sleepEndTime}`
    : '';
  ...
  if (resolvedScreenTimeHours != null) {
   if (hasBreakdown && breakdownTotal > resolvedScreenTimeHours) {
    throw badRequest('Сумма деталofации экранного времени не может превышать общее экранное время');
   }
  ...
  if (!ops.length) throw badRequest('No данных для сохранения');
...
router.post(
 ...
 asyncHandler(async (req: any, res) => {
  ...
  if (
   ...
   throw badRequest('Tueоричная роль должна отличаться от основной и быть корректной');
  }
