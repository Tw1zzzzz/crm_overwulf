...
const aiAnalysisLimit = rateLimit({
 ...
 message: {
  success: false,
  message: 'Слишком много запросов к AI-аналofу. Попробуйте через час.',
 },
 ...
 * Аналof состояния текущего авторofованного пользователя
 */
router.get('/analyze', protect, aiAnalysisLimit, async (req: any, res) => {
 try {
  ...
  return res.status(500).json({
   success: false,
   message: 'Не удалось сформировать аналof состояния. Попробуйте позже.',
  });
 }
});
 ...
 * Аналof состояния конкретного игрока (только для staff)
 */
router.get('/analyze/:playerId', protect, isStaff, aiAnalysisLimit, async (req: any, res) => {
 try {
  ...
  if (!playerId || playerId === 'undefined' || playerId === 'null') {
   return res.status(400).json({ success: false, message: 'Invalid ID игрока' });
  }
