...
const teamLogoUpload = multer({
 ...
 fileFilter: (_req, file, cb) => {
  ...
  cb(new Error('Разрешены только ofображения форматов: .jpg, .jpeg, .png, .webp, .svg'));
 }
});
...
router.post('/:id/regenerate-player-code', protect, isStaff, async (req: any, res) => {
 try {
  ...
  if (!canManageTeam(team, req.user)) {
   return res.status(403).json({ message: 'No прав на управление этой командой' });
  }
  ...
  console.error('[Teams] Ошибка ротации player codeа:', error);
...
router.post('/:id/regenerate-staff-code', protect, isStaff, async (req: any, res) => {
 try {
  ...
  console.error('[Teams] Ошибка ротации staff codeа:', error);
...
router.get('/:id/members', protect, isStaff, async (req: any, res) => {
 try {
  ...
  if (!canManageTeam(team, req.user)) {
   return res.status(403).json({ message: 'No прав на просмотр этой команды' });
  }
...
router.patch('/:id/branding', protect, isStaff, teamLogoUpload.single('logo'), async (req: any, res) => {
 try {
  ...
  if (!hasNameUpdate && !hasLogoUpdate) {
   return res.status(400).json({ message: 'No данных для обновления брендинга команды' });
  }
  ...
  return res.status(500).json({ message: 'Failed to update team branding' });
 }
});
