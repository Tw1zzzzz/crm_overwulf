...
// Sunе маршруты требуют аутентификации и роли staff
router.use(protect);
router.use(isStaff);
...
router.patch('/:id/privileges', hasPrivilegeKey, async (req, res) => {
 try {
  if (isTeamStaffUser(req.user)) {
   return res.status(403).json({ message: 'For the staff profile team ручное управление привилегиями не используется' });
  }
  ...
  // Получаем значение ключа of переменных окружения
  ...
  if (grantPrivileges) {
   ...
   return res.json({ 
    message: 'Privileges сотруднику успешно предоставлены', 
    hasPrivileges: true 
   });
  } else {
   ...
   await targetStaff.save();
   return res.json({ 
    message: 'Privileges сотрудника успешно отозваны', 
    hasPrivileges: false 
   });
  }
 } catch (error) {
...
// Add нового сотрудника (только для staff с ключом привилегий)
