 ...
 try {
  ...
  avatarUpload.single('avatar')(req, res, async (err) => {
   ...
   try {
    ...
    console.log(`- Name файла: ${filename}`);
    console.log(`- Относительный путь: ${avatarPath}`);
    console.log(`- Фofический путь: ${diskPath}`);
    
    // Проверяем существование загруженного файла
    const fileExists = fs.existsSync(diskPath);
    console.log(`📄 Файл существует в фofической директории: ${fileExists}`);
    ...
    console.log(`✅ Avatar пользователя ${updatedUser.name} успешно обновлен:`, avatarPath);
    
    // Возвращаем полную информацию о пользователе вместе с обновленным аватаром
    res.status(200).json({ 
     message: 'Avatar успешно обновлен',
     avatar: avatarPath,
     user: updatedUser
    });
   } catch (error) {
   ...
   if (!currentUser) {
    ...
    if (currentUser.avatar) {
     try {
      ...
      if (!avatarExists) {
       result.errors.push(`Avatar пользователя не найден по пути: ${avatarFullPath}`);
      }
     } catch (err) {
  ...
  for (const user of users) {
   ...
   if (user.avatar) {
    try {
     ...
     if (!avatarExists) {
      // Avatar не существует, помечаем как ошибку
      result.status = 'error';
...
router.get('/faceit-url', protect, async (req, res) => {
 try {
  if (!req.user || !req.user._id) {
   return res.status(401).json({ success: false, message: 'Не авторofован' });
  }
