...
// Для сотрудников: получить статистику настроения всех players
router.get('/players/mood', protect, isStaff, async (req: any, res) => {
 try {
  ...
  allMoodEntries.forEach(entry => {
   ...
   const userName = (entry.userId as any).name || 'Unknown player';
...
// Для сотрудников: получить статистику сна всех players
...
router.get('/players/:playerId/activity', protect, isStaff, async (req: any, res) => {
 try {
  ...
  try {
   // Извлекаем ID игрока of параметра
   // Проверяем, если playerId содержит объект игрока, как в URL of логов ошибок
   let actualPlayerId = playerId;
   
   // Если playerId содержит JSON-строку с объектом игрока
   if (playerId.includes('_id:') || playerId.includes('ObjectId')) {
    try {
     // Извлечение ID of строки
...
// Для сотрудников: получить статистику тестов всех players
...
// Для сотрудников: получить статистику колес баланса всех players
router.get('/players/balance-wheel', protect, isStaff, async (req: any, res) => {
 try {
  ...
  allWheels.forEach(wheel => {
   ...
   const userName = (wheel.userId as any).name || 'Unknown player';
...
router.get('/analytics/mood', protect, async (req: any, res) => {
 try {
  ...
  if (req.user.role === 'staff') {
   ...
   // Для players: получаем только их собственные данные
   const userMoodEntries = await MoodEntry.find({ userId: req.user._id }).sort({ date: -1 });
...
// Summary влияния состояния игрока на результаты тестов
