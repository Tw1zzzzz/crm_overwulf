...
router.post('/subscriptions/grant-user', async (req, res) => {
 try {
  ...
  return res.status(500).json({ message: 'Failed to grant plan пользователю' });
 }
});
...
router.post('/subscriptions/grant-team', async (req, res) => {
 try {
  ...
  return res.status(500).json({ message: 'Failed to grant plan команде' });
 }
});
...
router.post('/users/:id/send-password-reset', async (req, res) => {
 try {
  ...
  return res.status(503).json({ message: 'Failed to send email для сброса пароля' });
 }
});
