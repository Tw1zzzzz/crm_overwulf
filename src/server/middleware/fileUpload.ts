...
const fileFilter = (_req: any, file: Express.Multer.File, callback: multer.FileFilterCallback) => {
 ...
 if (allowedMimeTypes.includes(file.mimetype) && allowedExtensions.includes(fileExtension)) {
  ...
  callback(new Error(`Неподдерживаемый тип файла: ${file.mimetype}. Разрешены: ofображения (jpg, png, gif, webp), документы (pdf, doc, docx, xls, xlsx, txt, csv)`));
 }
};
...
export const uploadReportFiles = multer({
 ...
 limits: {
  ...
  files: 5 // Maximum 5 файлов за раз
 }
 ...
 if (error instanceof multer.MulterError) {
  ...
  switch (error.code) {
   ...
   case 'LIMIT_FILE_COUNT':
    message = 'Слишком много файлов. Maximum: 5 файлов';
    break;
...
/**
 * Утилита для перемещения файлов of temp в постоянную директорию
 */
export const moveFilesToReports = async (files: Express.Multer.File[]): Promise<string[]> => {
 ...
 for (const file of files) {
  ...
  try {
   // Перемещаем файл of temp в постоянную директорию
 ...
 * Add в server.ts: app.use('/uploads/team-reports', express.static(UPLOAD_PATHS.REPORTS));
 */
export { UPLOAD_PATHS }; 