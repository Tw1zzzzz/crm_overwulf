...
export const protect = async (req: any, res: any, next: any) => {
 ...
 console.log(`[AUTH] Проверка авторofации для ${req.method} ${req.originalUrl}`);
 console.log(`[AUTH] Заголовки:`, req.headers.authorization ? 'Bearer token присутствует' : 'Title авторofации отсутствует');

 // Проверяем наличие токена в заголовке Authorization
 if (req.headers.authorization && req.headers.authorization.startsWith('Bearer')) {
  try {
   // Получаем токен of заголовка
   ...
   // Получаем пользователя of базы, исключая пароль
   ...
   if (decoded.iat && req.user.passwordChangedAt instanceof Date) {
    ...
    if (tokenIssuedAtMs < passwordChangedAtMs) {
     ...
     return res.status(401).json({
      message: 'Session устарела после смены пароля. Выполните вход повторно.',
      code: 'TOKEN_STALE'
     });
    }
   }
   
   console.log(`[AUTH] Пользователь ${req.user.name} (${req.user.role}) авторofован`);
   next();
  } catch (error) {
  ...
  return res.status(401).json({ message: 'Не авторofован, токен отсутствует' });
 }
};
...
export const isStaff = (req: any, res: any, next: any) => {
 if (req.user && req.user.role === 'staff') {
  ...
  return res.status(403).json({ message: 'No прав доступа для этого действия' });
 }
};
  ...
  if (req.user && req.user.role === 'staff') {
   ...
   if (isTeamStaffUser(req.user) && !getScopedTeamId(req.user)) {
    return res.status(403).json({
     message: 'Сначала создайте команду или присоединитесь к существующей по staff codeу',
     requiresTeamSetup: true
    });
   }

   // Получаем корректный ключ привилегий of переменных окружения
   const validPrivilegeKey = process.env.STAFF_PRIVILEGE_KEY;
   
   console.log('[AUTH] Проверка привилегий: ключ of env загружен:', !!process.env.STAFF_PRIVILEGE_KEY);
   ...
   if (req.user.privilegeKey && req.user.privilegeKey === validPrivilegeKey) {
    ...
    return res.status(403).json({ 
     message: 'No доступа для редактирования состава участников. Требуется действительный ключ доступа для staff.',
   ...
   console.log(`[AUTH] Доступ запрещен: требуется роль 'staff'`);
   return res.status(403).json({ 
    message: 'No прав доступа для этого действия' 
   });
  }
 } catch (error) {
...
export const hasCorrelationAnalysisSubscription = async (req: any, res: any, next: any) => {
 try {
  ...
  return res.status(403).json({
   message: 'Требуется активная подписка "Корреляционный аналof"',
   requiresSubscription: true,
   requiredPlan: 'Корреляционный аналof',
  });
 } catch (error) {
  console.error('[AUTH] Ошибка при проверке подписки Корреляционный аналof:', error);
  return res.status(500).json({ message: 'Ошибка сервера при проверке подписки' });
 }
};
...
export const hasGameStatsSubscription = async (req: any, res: any, next: any) => {
 try {
  ...
  return res.status(403).json({
   message: 'Требуется активная подписка "Game statistics"',
   requiresSubscription: true,
   requiredPlan: 'Game statistics',
  });
 } catch (error) {
  console.error('[AUTH] Ошибка при проверке подписки Game statistics:', error);
  return res.status(500).json({ message: 'Ошибка сервера при проверке подписки' });
 }
};
