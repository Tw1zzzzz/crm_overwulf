 ...
 * Использует patterns of Context7 документации express-validator
 */
export const validate = (validations: ValidationChain[]) => {
 return async (req: express.Request, res: express.Response, next: express.NextFunction) => {
  console.log('🔍 [Validation] Начало валидации. Data req.body:', JSON.stringify(req.body, null, 2));
...
export const validateCreateReport = [
 body('title')
  .notEmpty()
  .withMessage('Name отчета обязательно')
  .isLength({ min: 3, max: 200 })
  .withMessage('Name должно быть от 3 до 200 символов')
  .trim()
  .escape(),
 ...
 body('type')
  ...
  .withMessage(`Type отчета должен быть одним of: ${Object.values(ReportType).join(', ')}`),

 body('visibility')
  .optional()
  .isIn(Object.values(ReportVisibility))
  .withMessage(`Видимость должна быть одной of: ${Object.values(ReportVisibility).join(', ')}`),
 ...
 body('content.sections')
  .isArray({ min: 1 })
  .withMessage('Report должен содержать как минимум одну секцию'),

 body('content.sections.*.title')
  .notEmpty()
  .withMessage('Name секции обязательно')
  .isLength({ max: 200 })
  .withMessage('Name секции не может превышать 200 символов'),
 ...
 body('content.sections.*.type')
  ...
  .withMessage('Type секции должен быть одним of: text, markdown, chart, table'),
...
export const validateUpdateReport = [
 ...
 body('title')
  .optional()
  .notEmpty()
  .withMessage('Name отчета не может быть пустым')
  .isLength({ min: 3, max: 200 })
  .withMessage('Name должно быть от 3 до 200 символов')
  .trim()
  .escape(),
...
/**
 * Валидация для ofменения статуса отчета
 */
export const validateUpdateReportStatus = [
 ...
 body('status')
  ...
  .withMessage(`Статус должен быть одним of: ${Object.values(ReportStatus).join(', ')}`)
];
...
export const validateSupportRequest = [
 body('name')
  .optional()
  .isLength({ max: 100 })
  .withMessage('Name не должно превышать 100 символов')
  .trim(),
 ...
 body('category')
  .notEmpty()
  .withMessage('Category обращения обязательна')
  .isIn(['access', 'bug', 'billing', 'integration', 'other'])
  .withMessage('Указана неofвестная категория обращения'),
...
/**
 * Middleware для санитofации HTML контента от XSS атак
 * Очищает потенциально опасные HTML теги of текстовых полей
 */
export const sanitizeHtmlContent = (req: express.Request, res: express.Response, next: express.NextFunction) => {
 try {
  console.log('🧹 [SanitizeHTML] Санитofация HTML контента');

  // Функция для безопасной санитofации текста
  const sanitizeText = (text: string): string => {
   if (typeof text !== 'string') return text;
   
   // Простая санитofация - удаляем потенциально опасные теги
   return text
  ...
  // Санитofируем текстовые поля в req.body
  if (req.body.title) {
  ...
  // Санитofируем содержимое отчета
  if (req.body.content) {
  ...
  console.log('✅ [SanitizeHTML] HTML контент санитofирован');
  next();
 } catch (error) {
  console.error('❌ [SanitizeHTML] Ошибка санитofации:', error);
  res.status(500).json({
   status: 'error',
