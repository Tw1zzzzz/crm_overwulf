...
const extractPlayerId = (playerId: any): string => {
 ...
 if (typeof playerId === 'string') {
  ...
  if (objectIdMatch && objectIdMatch[1]) {
   console.log('Extracted ID from ObjectId string:', objectIdMatch[1]);
  ...
  if (jsonIdMatch && jsonIdMatch[2]) {
   console.log('Extracted ID from JSON string:', jsonIdMatch[2]);
  ...
  try {
   if (playerId.includes('{') && playerId.includes('}')) {
    ...
    if (jsonObj && jsonObj._id) {
     console.log('Extracted ID from parsed JSON:', jsonObj._id);
     return jsonObj._id;
    }
   }
  } catch (error) {
   console.error('Error while parsing JSON string:', error);
  }
 }
 
 // Возвращаем исходное значение, если не удалось ofвлечь ID
 return playerId;
}
...
const MoodTracker = () => {
 ...
 // Новые переменные состояния для статистики players (для персонала)
 ...
 useEffect(() => {
  if (user?.role === "staff") {
   // Staff видит только статистику players
   loadPlayerStats();
   loadRecentEntries();
   ...
   try {
    ...
    if (savedPlayerId) {
     // Извлекаем ID of возможного объекта или строки объекта
    ...
    console.error('Error while restoring player ID from sessionStorage:', e);
   }
  } else {
   // Players видят свои entries
   loadEntries();
  }
 ...
 const loadPlayerStats = async (dateOverride?: Date) => {
  ...
  try {
   ...
   console.error("Error while loading player statistics:", error);
   ...
   try {
    ...
    console.error("Error while loading player statistics (fallback method):", fallbackError);
    toast({
     title: "Loading error",
 ...
 const loadRecentEntries = async () => {
  ...
  try {
   ...
   console.error("Error while loading latest player entries:", error);
   setRecentEntries([]);
  } finally {
 ...
 // Method для changes выбранной даты графика
 ...
 const loadPlayerEntriesForDate = async (playerId: string | any, date: Date) => {
  ...
  try {
   ...
   if (!actualPlayerId || actualPlayerId === 'undefined' || actualPlayerId === 'null') {
    toast({
     title: "Loading error",
     description: "Invalid player identifier.",
     variant: "destructive"
    });
    setIsLoadingPlayerData(false);
    return;
   ...
   try {
    // Formatируем дату для API (YYYY-MM-DD)
    ...
    try {
     ...
     // Фильтруем entries по дате на стороне клиента
     const apiDateFormat = toLocalYmd(date);
     ...
     // Генерируем данные для графика of доступных entries
     ...
     if (error.response) {
      if (error.response.status === 400) {
       errorMessage = "Invalid player identifier.";
      } else if (error.response.status === 404) {
       errorMessage = "Player not found.";
      } else if (error.response.status === 500) {
       errorMessage = "Server error while loading data.";
      }
     }
   ...
   try {
    ...
    if (typeof actualPlayerId === 'string' && /^[0-9a-fA-F]{24}$/.test(actualPlayerId)) {
     ...
     console.log('Player ID saved in sessionStorage:', actualPlayerId);
    } else {
     console.error('Attempted to save invalid ID to sessionStorage:', actualPlayerId);
    }
   } catch (e) {
    // Игнорируем ошибки sessionStorage
    console.error('Error while saving ID to sessionStorage:', e);
   }
   ...
   toast({
    title: "Loading error",
    description: "An unknown error occurred while trying to load data.",
    variant: "destructive"
   });
  } finally {
   setIsLoadingPlayerData(false);
 ...
 const loadPlayerEntries = async (playerId: string | any) => {
  ...
  // Проверяем, передан ли объект вместо ID
  let actualPlayerId = extractPlayerId(playerId);
  console.log('Extracted player ID for loading entries:', actualPlayerId);
  ...
  try {
   ...
   toast({
    title: "Loading...",
    description: "Loading player records",
    variant: "default"
   });
 ...
 // Подготовка данных для графиков - теперь используется как запасной вариант и принимает entries как параметр
 const prepareChartData = (entries = playerEntries) => {
  if (entries.length === 0) return;
  
  // Сортируем entries по дате (от старых к новым)
  const sortedEntries = [...entries].sort((a, b) => 
   new Date(a.date).getTime() - new Date(b.date).getTime()
  );
  
  // Создаем Map для группировки entries по дате
  const entriesByDate = new Map<string, { moodSum: number, energySum: number, count: number }>();
  
  // Группируем entries по дате и считаем средние значения
  ...
  try {
   ...
   if (user) {
    ...
    try {
     ...
     console.log('Records loaded from server:', serverEntries.length);
     
     // Удаляем дубликаты entries с одинаковыми датами и временем дня
     const uniqueEntries = removeDuplicateEntries(serverEntries);
     console.log('Unique entries after processing:', uniqueEntries.length);
 ...
 // Sunпомогательная функция для удаления дубликатов entries
 const removeDuplicateEntries = (entries: any[]) => {
  ...
  entries.forEach(entry => {
   ...
   // Создаем уникальный ключ для entries на основе даты и времени суток
   const key = `${entryDate}_${entry.timeOfDay}`;
   
   // Если entry с таким ключом уже существует, перезаписываем её только если текущая entry новее
  ...
  // Возвращаем массив уникальных entries
 ...
 const handleSubmit = async () => {
  // Staff не должен иметь возможность создавать entries
  if (user?.role === "staff") {
   toast({
    title: "Access denied",
  ...
  try {
   ...
   // Formatируем дату для проверки и API
   ...
   // Проверяем, есть ли уже entry на выбранную дату и время суток
   ...
   if (existingEntries.length > 0) {
    toast({
     title: "Attention",
     description: `An entry already exists for ${formatDate(selectedDate)} (${formatTimeOfDay(timeOfDay)}). Entries for the selected time of day must not be duplicated.`,
     variant: "destructive"
    });
    setIsLoading(false);
    return;
   ...
   // Создаем уникальный идентификатор для новой entries
   const tempId = `${formattedDate}_${timeOfDay}_${Date.now()}`;
   
   // Создаем entry локально сначала с временным уникальным ID
   ...
   if (user) {
    try {
     ...
     console.log('Entry saved on the server:', response.data);
     
     // Обновляем локальную entry с ID от сервера
     if (response.data && (response.data.id || response.data._id)) {
      const serverId = response.data.id || response.data._id;
      console.log("ID received from server:", serverId);
      
      // Удаляем entry с временным ID
      moodRepository.delete(savedEntry.id);
      
      // Создаем новую entry с ID сервера
      savedEntry = {
       ...savedEntry,
       id: serverId
      };
      
      // Сохраняем обновленную entry в локальное хранилище
      const allEntries = moodRepository.getAll();
      moodRepository.updateFromServer([...allEntries, savedEntry]);
     }
    } catch (error) {
     console.error('Server save error (will sync later):', error);
    }
   }
   
   // Загружаем entries заново
   await loadEntries();
   resetForm();
 ...
 const handleDelete = async (id: string | undefined) => {
  ...
  if (!id) {
   console.error("Error: ID is not defined when trying to delete");
   toast({
    title: "Error",
    description: "Error: entry ID is not defined",
    variant: "destructive"
   });
   return;
  }
  ...
  try {
   // Удаляем entry of API
   const response = await deleteMoodEntry(id);
   console.log(`API response on deletion:`, response.data);
 ...
 const getDayEntries = (date: Date) => {
  // Возвращаем entries в зависимости от выбранного режима просмотра
 ...
 // Получение entries для конкретного времени дня (с учетом выбранного режима просмотра)
 const getTimeOfDayEntries = (date: Date, time: "morning" | "afternoon" | "evening") => {
  const dayEntries = getDayEntries(date);
  
  // Step 1: Создаем Set для хранения уникальных ID
  const uniqueIds = new Set();
  
  // Step 2: Фильтруем entries, сначала по времени дня, затем по уникальности ID
  const timeEntries = dayEntries
   .filter((entry: any) => entry.timeOfDay === time)
   .reduce((unique: MoodEntryWithTimeOfDay[], entry: any) => {
    // Генерируем идентификатор для entries, основанный на нескольких ее свойствах
    const entryId = entry.id || entry._id || `${entry.date}_${entry.timeOfDay}_${entry.mood}_${entry.energy}`;
    
    // Проверяем, есть ли уже entry с таким ID
    if (!uniqueIds.has(entryId)) {
     // Если нет, добавляем ID в Set и entry в результат
  ...
  if (timeEntries.length > 0) {
   console.log(`Unique entries for ${time} для ${formatDate(date)}:`, timeEntries.length);
  }
   ...
   return (
    ...
    >
     ...
     <p style={{ margin: '0 0 5px' }}>
      {data.time ? `Time: ${data.time}` : 'Time not specified'}
  ...
  useEffect(() => {
   const loadActivityData = async () => {
    try {
     ...
     // Проверяем, передан ли объект вместо ID
     let actualPlayerId = extractPlayerId(playerId);
     console.log('Player ID for loading activity data:', actualPlayerId);
     ...
     if (response.data && Array.isArray(response.data) && response.data.length > 0) {
      ...
      setActivityData([]);
      console.log('API returned empty activity chart data');
     }
    } catch (error) {
     console.error('Error while loading activity data:', error);
     setActivityData([]);
 ...
 // Sunпомогательная функция для устранения дубликатов entries
 const getUniqueEntriesByTimeOfDay = (entries: MoodEntryWithTimeOfDay[], timeOfDay: "morning" | "afternoon" | "evening") => {
  // Объект для отслеживания уникальных ID entries
  const uniqueIDs = new Set<string>();
  // Объект для отслеживания уникальных комбинаций дата+время суток (чтобы ofбежать дубликатов)
  ...
  return entries
   ...
   .filter(entry => {
    ...
    // Получаем ID entries или генерируем его
 ...
 const timeOfDayConfig: Record<TimeOfDayKey, {
  ...
  morning: {
   label: "Morning",
   hint: "A quick start-of-day check helps show how you enter the training rhythm.",
   emptyTitle: "Morning slot is still empty",
   emptyCopy: "Add the first snapshot after waking up to see what resource the day starts with.",
   accent: "#4EA1FF",
  ...
  afternoon: {
   label: "Afternoon",
   hint: "The daytime mark shows how load and tasks affect your condition during the day.",
   emptyTitle: "Daytime slot is empty",
   emptyCopy: "Add a short note after the first half of the day to track a dip or peak.",
   accent: "#00D7A3",
  ...
  evening: {
   label: "Evening",
   hint: "The evening snapshot helps close the day and understand how recovery affects overall dynamics.",
   emptyTitle: "Evening slot is still empty",
   emptyCopy: "Log your state after the day ends to see the final recovery picture.",
   accent: "#A46CFF",
 ...
 const getEntryCountLabel = (count: number): string => {
  if (count % 10 === 1 && count % 100 !== 11) return "entry";
  if (count % 10 >= 2 && count % 10 <= 4 && (count % 100 < 12 || count % 100 > 14)) return "entries";
  return "entries";
 };

 const getScoreDescriptor = (value: number, metric: "mood" | "energy"): string => {
  if (metric === "mood") {
   if (value >= 9) return "Excellent baseline";
   if (value >= 7) return "Confident state";
   if (value >= 5) return "Working balance";
   if (value >= 3) return "There is a drop";
   return "Need a pause";
  }

  if (value >= 9) return "Maximum charge";
  if (value >= 7) return "High resource";
  if (value >= 5) return "Steady pace";
  if (value >= 3) return "Resource is reduced";
   ...
   >
    <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
     <div className="flex items-start gap-3">
      ...
      <div className="space-y-2">
       <div className="flex flex-wrap items-center gap-2">
        ...
        >
         {timeEntries.length > 0 ? `${timeEntries.length} ${getEntryCountLabel(timeEntries.length)}` : "Empty for now"}
        </span>
       </div>
       <p className="text-sm leading-6" style={{ color: COLORS.textColorSecondary }}>
        {slotConfig.hint}
     ...
     >
      ...
      {timeEntries.length > 0 ? "Add more" : "Fill slot"}
     </Button>
    </div>
 ...
 return (
  <div className="space-y-4" style={containerStyle}>
   ...
   {user?.role === "staff" ? (
    // Для персонала отображаем статистику players
    <div className="space-y-4">
     {isLoadingPlayerData ? (
      <Card style={cardStyle}>
       <CardContent className="pt-6">
        <div className="flex justify-center py-8">
         <p style={descriptionStyle}>Loading data...</p>
        </div>
       </CardContent>
      </Card>
     ) : playerStats.length > 0 ? (
      <>
       {/* Добавляем общий селектор даты для всех players */}
       <Card style={cardStyle}>
        <CardContent className="py-4">
         <div className="flex justify-between items-center">
          <h3 className="text-xl font-semibold" style={titleStyle}>
           Mood and energy на дату
          </h3>
          <div className="flex items-center space-x-2">
           <Button 
            variant="ghost" 
        ...
        {playerStats.map(player => (
         <Card key={player.userId} style={cardStyle}>
          <CardHeader className="pb-2">
           ...
           <CardDescription style={descriptionStyle}>
            Averageние показатели: Mood {player.mood.toFixed(1)}, Energy {player.energy.toFixed(1)}
           </CardDescription>
          </CardHeader>
          <CardContent className="pb-0">
           ...
           <div className="mt-2">
            <p className="text-sm mb-1" style={descriptionStyle}>Records</p>
      ...
      <Card style={cardStyle}>
       <CardContent className="pt-6">
        <div className="flex justify-center py-8">
         <p style={descriptionStyle}>No data о настроении players</p>
        </div>
       </CardContent>
      </Card>
     )}
      ...
      <div className="space-y-4 mt-8" ref={playerEntriesRef}>
       <div className="flex justify-between items-center">
        <div>
         ...
         <p className="text-sm mt-1" style={descriptionStyle}>
          ...
          {playerStats.find(p => p.userId === selectedPlayerId)?.entries || 0} entries всего
         </p>
        </div>
        <Button 
         variant="outline" 
         ...
         onClick={() => {
          ...
          try {
           ...
           // Игнорируем ошибки sessionStorage
           console.error('Error while removing ID from sessionStorage:', e);
          }
         }}
        >
         Back to list
       ...
       {isLoadingPlayerData ? (
        ...
        <>
         ...
         <Card style={cardStyle}>
          <CardHeader>
           <CardTitle style={titleStyle}>Indicator dynamics</CardTitle>
           <CardDescription style={descriptionStyle}>
            Mood and energy change over time
           </CardDescription>
          </CardHeader>
          <CardContent>
           <div className="h-80">
         ...
         <Card style={cardStyle} className="mt-4">
          <CardHeader>
           <CardTitle style={titleStyle}>
            Records for {formatDate(selectedChartDate, "d MMMM yyyy")}
           </CardTitle>
           <CardDescription style={descriptionStyle}>
            Detailed player records with comments
           </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card style={cardStyle}>
         <CardContent className="pt-6">
          <div className="flex justify-center py-8">
           <p style={descriptionStyle}>This player has no mood entries</p>
          </div>
         </CardContent>
        </Card>
       )}
    ...
    // Для players отображаем их личные entries
    <div className="space-y-4">
     <div
       ...
       <div className="space-y-3">
        ...
        <div className="space-y-2">
         <h3 className="text-2xl font-semibold md:text-3xl" style={{ color: COLORS.textColor }}>
          Log the day quickly and keep dynamics under control
         </h3>
         <p className="max-w-2xl text-sm leading-7 md:text-base" style={{ color: "rgba(226, 232, 240, 0.82)" }}>
          Choose a day, fill morning, afternoon, and evening as the day goes, and leave short context if something noticeably affects your condition.
         </p>
        </div>
       </div>
      ...
      <CardHeader className="pb-0">
       <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
         ...
         <CardDescription style={descriptionStyle}>
          Choose a day and see which slots are already filled.
         </CardDescription>
        </div>
     ...
     {/* Отображение entries на выбранную дату с возможностью добавления */}
     <Card style={cardStyle}>
      <CardContent className="pt-6">
       <div
       ...
       >
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
         <div className="space-y-3">
          ...
          <div>
           <h3 className="text-2xl font-semibold" style={{ color: COLORS.textColor }}>
            Records for {formatDate(selectedDate, "d MMMM yyyy")}
           </h3>
           <p className="mt-2 max-w-2xl text-sm leading-7" style={{ color: COLORS.textColorSecondary }}>
            Each slot shows a separate state snapshot. Empty blocks can be filled in quickly from here.
           </p>
          </div>
         </div>
         <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          >
           <Plus className="mr-2 h-4 w-4" />
           {selectedDaySnapshot.nextMissingSlot ? `Add ${timeOfDayConfig[selectedDaySnapshot.nextMissingSlot].label.toLowerCase()}` : "Add entry"}
          </Button>
         </div>
        </div>
       </div>
   ...
   {/* Диалоговое окно для добавления entries - применяем стили диалога */}
    ...
    >
     ...
     <div className="flex min-h-0 flex-1 flex-col">
      <DialogHeader className="relative space-y-3 border-b px-6 pb-4 pt-6" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
       ...
       <DialogTitle style={titleStyle}>Add entry</DialogTitle>
       <DialogDescription style={descriptionStyle}>
        Record your state at the right moment of the day. Each slot can be filled only once, so the interface immediately suggests available periods.
       </DialogDescription>
      </DialogHeader>
        ...
        <div className="grid gap-4 lg:grid-cols-[1.12fr_0.88fr]">
         ...
         >
          ...
          {selectedSlotFilled && (
           ...
           >
            This period already has an entry. Choose another slot to save a new snapshot.
           </div>
          )}

          <div className="mt-4 space-y-2">
           <Label htmlFor="comment" style={{ color: COLORS.textColor }}>Comment</Label>
           <Textarea
            id="comment"
