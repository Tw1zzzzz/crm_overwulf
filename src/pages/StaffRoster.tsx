...
const getAllStaff = async () => {
 ...
 if (!response.ok) {
  ...
  throw new Error(error.message || 'Error while получении списка персонала');
 }
...
const updateStaffPrivileges = async (staffId: string, grantPrivileges: boolean) => {
 ...
 if (!response.ok) {
  const error = await response.json();
  throw new Error(error.message || 'Error while обновлении привилегий');
 }
...
const deleteStaffMember = async (staffId: string) => {
 ...
 if (!response.ok) {
  const error = await response.json();
  throw new Error(error.message || 'Error while удалении сотрудника');
 }
...
const createStaffMember = async (staffData: { name: string; email: string; password: string }) => {
 ...
 if (!response.ok) {
  const error = await response.json();
  throw new Error(error.message || 'Error while создании сотрудника');
 }
...
const StaffRoster = () => {
 ...
 const loadStaffMembers = async () => {
  try {
   ...
   setError('Error while загрузке персонала: ' + err.message);
 ...
 const handleTogglePrivileges = async (staffId: string, staffName: string, currentPrivileges: boolean) => {
  ...
  if (!window.confirm(`Are you sure you want to ${action} привилегии у сотрудника ${staffName}?`)) {
   return;
  }
  ...
  try {
   ...
   toast.success(`Privileges сотрудника ${staffName} успешно ${currentPrivileges ? 'отозваны' : 'предоставлены'}`);
  } catch (err: any) {
   console.error('Error updating privileges:', err);
   toast.error('Error while обновлении привилегий: ' + err.message);
  } finally {
   setIsLoading(false);
  }
 };
 ...
 const handleDeleteStaff = async (staffId: string, staffName: string) => {
  ...
  try {
   ...
   // Удаляем of локального списка
   setStaffMembers(prev => prev.filter(staff => staff.id !== staffId && staff._id !== staffId));
   
   toast.success(`Staff member ${staffName} успешно удален`);
  } catch (err: any) {
   console.error('Error deleting staff:', err);
   toast.error('Error while удалении сотрудника: ' + err.message);
  } finally {
   setIsLoading(false);
  }
 };
 ...
 const handleAddStaff = async (e: React.FormEvent) => {
  ...
  try {
   ...
   // Satрасываем форму и закрываем диалог
   setNewStaffData({ name: '', email: '', password: '' });
   setShowAddDialog(false);
   
   toast.success(`Staff member ${response.staff.name} успешно добавлен`);
  } catch (err: any) {
   console.error('Error adding staff:', err);
   toast.error('Error while добавлении сотрудника: ' + err.message);
  } finally {
 ...
 if (checkingPrivilege) {
  return (
   <div className="container mx-auto py-4">
    <h1 className="text-2xl font-bold mb-4" style={{ color: COLORS.textColor }}>Staff management</h1>
   ...
   <div className="container mx-auto py-4">
    ...
    <Card style={{ backgroundColor: COLORS.cardBackground, borderColor: COLORS.borderColor }}>
     <CardHeader>
      ...
      <CardDescription style={{ color: COLORS.textColorSecondary }}>
       {isTeamStaff
        ? "Create a team first or join one with a staff code"
        : "A staff access key is required to manage staff roster"}
      </CardDescription>
     </CardHeader>
     <CardContent className="py-4">
      <div className="flex flex-col items-center gap-4 py-6">
       ...
       <p className="text-center" style={{ color: COLORS.textColor }}>
        {isTeamStaff
         ? "Until a team is linked, the staff list and visible players will be empty."
         : "To access staff management, add a staff access key in your profile."}
       </p>
       <Button 
 ...
 return (
  <div className="container mx-auto py-4">
   <div className="flex justify-between items-center mb-4">
    <h1 className="text-2xl font-bold" style={{ color: COLORS.textColor }}>Staff management</h1>
     ...
     <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
      ...
      >
       <DialogHeader className="text-center pb-4">
        ...
        >
         Enter data to create a new team staff member
        </DialogDescription>
       </DialogHeader>
   ...
   <Card style={{ backgroundColor: COLORS.cardBackground, borderColor: COLORS.borderColor }}>
     ...
     <CardTitle style={{ color: COLORS.textColor }}>Staff team</CardTitle>
     <CardDescription style={{ color: COLORS.textColorSecondary }}>
      {isTeamStaff ? 'Manage staff roster своей team' : 'Manage staff roster и их привилегиями'}
     </CardDescription>
    </CardHeader>
    <CardContent>
     {isLoading ? (
