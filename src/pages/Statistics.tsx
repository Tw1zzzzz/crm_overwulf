...
// Компонент для стилofованных всплывающих подсказок на графиках
...
const Statistics = () => {
 ...
 const loadPersonalData = async () => {
  try {
   console.log('Loading personal player statistics');
   setLoadingPlayersData(true);
   setLoadingError(null);
   
   // Получаем данные о настроении of API
   ...
   if (moodResponse.data && Array.isArray(moodResponse.data)) {
    console.log(`Received ${moodResponse.data.length} entries о настроении`);
    setMoodEntries(moodResponse.data);
   } else {
    console.warn('No mood data received');
    setMoodEntries([]);
   }

   if (sleepResponse.data && Array.isArray(sleepResponse.data)) {
    console.log(`Received ${sleepResponse.data.length} entries о сне`);
    setSleepEntries(sleepResponse.data);
   } else {
    console.warn('No sleep data received');
    setSleepEntries([]);
   }

   if (testResponse.data && Array.isArray(testResponse.data)) {
    console.log(`Received ${testResponse.data.length} entries о тестах`);
    setTestEntries(testResponse.data);
   } else {
    console.warn('No test data received');
    setTestEntries([]);
   }
  } catch (error) {
   console.error('Error while loading data:', error);
   setLoadingError(`Data loading error: ${(error as Error).message}`);
   
   // В случае ошибки API, пытаемся получить данные of локального хранилища как запасной вариант
 ...
 const fetchPlayerStats = async (playerId: string) => {
  ...
  try {
   ...
   if (typeof playerId === 'object' && playerId !== null) {
    console.log('Received player object instead of ID, extracting ID:', playerId);
    // @ts-ignore
    playerIdStr = playerId._id || '';
    if (!playerIdStr) {
     throw new Error('Invalid player ID format: ID is missing from object');
    }
   }
 ...
 const loadPlayersData = async () => {
  try {
   ...
   // Загружаем данные о настроении players
   const [moodResponse, sleepResponse, testResponse] = await Promise.all([
 ...
 return (
  <div className="space-y-4">
   <div className="mb-2 flex items-center justify-between">
    ...
    >
     ...
     <SelectContent className="bg-[#1C1F3B] border-[#293056] text-white">
      <SelectItem value="personal">Player statistics</SelectItem>
      <SelectItem value="players">Overall statistics</SelectItem>
     </SelectContent>
    </Select>
   </div>
   ...
   {activeTab === "personal" ? (
    ...
    // Показываем общую статистику всех players
    <PlayersStats
