...
const TopPlayers: React.FC = () => {
   ...
   <Container maxWidth="md" sx={{ mt: 6 }}>
    <Box sx={{ p: 3, borderRadius: 2, backgroundColor: theme.palette.background.paper }}>
     ...
     <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
      � ейтинг players доступен только для участников команд.
     </Typography>
 ...
 // Обработчик changes текстового поля
 ...
 const handleSubmit = async () => {
  ...
  try {
   ...
   console.error('Error while updating rating:', err);
   setError('Failed to update rating. Please check authorization and try again.');
  } finally {
   setLoading(false);
  }
 };
 ...
 const handleResetRating = async () => {
  ...
  if (!confirm(`Are you sure you want to reset player rating ${selectedPlayer.name}?`)) {
   return;
  }
  ...
  try {
   ...
   console.error('Error while resetting rating:', err);
   setError('Failed to reset rating. Please check authorization and try again.');
  } finally {
   setLoading(false);
  }
 };
  ...
  <Container maxWidth="lg" sx={{ mt: 3, mb: 3 }}>
   ...
   {selectedPlayer && (
    <Dialog open={!!selectedPlayer} onClose={handleCloseDialog}>
     <DialogTitle>
      Player rating change: {selectedPlayer.name}
     </DialogTitle>
     <DialogContent>
...
// Компонент для отображения таблицы players
   ...
   <Table size="medium" sx={{ '& .MuiTableCell-root': { py: 2, px: 2.5 } }}>
    <TableHead>
     <TableRow sx={{ backgroundColor: '#3c83f6', color: 'white' }}>
      ...
      <TableCell align="right" sx={{ color: 'white', fontWeight: 'bold' }}>Game points</TableCell>
      <TableCell align="right" sx={{ color: 'white', fontWeight: 'bold' }}>Out-of-game</TableCell>
      <TableCell align="right" sx={{ color: 'white', fontWeight: 'bold' }}>Discipline</TableCell>
      {isStaff && <TableCell align="right" sx={{ color: 'white', fontWeight: 'bold' }}>Actions</TableCell>}
     </TableRow>
    </TableHead>
    <TableBody>
     {players.length === 0 ? (
