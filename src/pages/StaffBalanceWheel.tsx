...
const StaffBalanceWheel = () => {
 ...
 useEffect(() => {
  const fetchPlayers = async () => {
   try {
    ...
    setPlayersDataError(false); // Satрасываем флаг ошибки в начале
    
    const response = await getPlayers();
    
    if (response.data && Array.isArray(response.data) && response.data.length > 0) {
     // Убедимся, что у всех players есть id
     ...
     if (validPlayers.length > 0) {
      ...
      setApiError("Team players not found");
     }
    } else {
     setPlayers([]);
     setSelectedPlayerId("");
 ...
 // Загрузка данных колес баланса всех players
 useEffect(() => {
  const fetchAllPlayersWheelData = async () => {
   try {
    console.log("[DEBUG] Loading balance wheel data for all players...");
    setLoading(true);
    setApiError(null);
    
    const response = await getAllPlayersBalanceWheelStats();
    console.log("[DEBUG] Loaded balance wheel data for all players:", response);
    
    if (response.data && Array.isArray(response.data)) {
     if (response.data.length > 0) {
      // Обработка данных всех players
      const processedData = response.data.map(playerData => {
       // Проверка и ofвлечение userId of строки, если это JSON строка
       let userId = playerData.userId;
       if (typeof userId === 'string' && userId.includes('_id:')) {
        try {
         // Пытаемся ofвлечь _id of строки JSON
         const match = userId.match(/ObjectId\(["']([^"']+)["']\)/);
         if (match && match[1]) {
          userId = match[1];
         }
        } catch (e) {
         console.error("[DEBUG] Error while processing userId:", e);
        }
       }
      ...
      console.log("[DEBUG] Processed balance wheel data:", processedData);
      setAllPlayersWheelData(processedData);
      setLastRefreshTime(Date.now());
      setPlayersDataError(false); // Satрасываем флаг ошибки, так как данные получены
      
     } else {
      console.log("[DEBUG] No balance wheel data");
      // Не устанавливаем тестовые данные, а просто показываем сообщение об ошибке
      setApiError("No balance wheel data");
      setPlayersDataError(true);
     }
    } else {
     console.log("[DEBUG] Received invalid balance wheel data");
     setApiError("Received invalid balance wheel data");
     setPlayersDataError(true);
    }
   } catch (error) {
    console.error("[DEBUG] Error while loading balance wheel data for all players:", error);
 ...
 const forceFetchBalanceWheelForPlayer = async (playerId: string) => {
  try {
   ...
   // Allгда делаем прямой запрос к API
   ...
   if (response && response.data) {
    ...
    if (wheelData && wheelData.length > 0) {
     ...
     // Обновляем данные в общем массиве всех players
   ...
   setApiError(`Failed to get balance wheel data for player ${playerId}`);
   setPlayersDataError(true);
  ...
  try {
   ...
   if (response && response.data) {
    ...
    if (wheelData && wheelData.length > 0) {
     ...
     toast.error("Failed to get balance wheel data");
    }
   } else {
   ...
   console.error("Error while updating player data:", error);
 ...
 const handlePlayerSelect = (value: string | null) => {
  ...
  // Используем существующие данные of общего массива
  const playerData = allPlayersWheelData.find(data => 
 ...
 if (user?.role !== "staff") {
  return (
   <div className="container mx-auto py-6">
    ...
    <p>This section is available only to staff</p>
   </div>
  );
 }
