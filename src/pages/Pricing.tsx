...
const Pricing = () => {
 ...
 const purchaseMutation = useMutation({
  ...
  onError: (mutationError) => {
   const message = mutationError instanceof Error ? mutationError.message : 'Failed to create invoice';
   toast.error(message);
  },
 });
 ...
 return (
  <div className="container mx-auto py-6 space-y-6">
   ...
   <Alert>
    ...
    <AlertDescription>
     {user?.hasPerformanceCoachCrmAccess || user?.hasCorrelationAnalysisAccess || user?.hasGameStatsAccess
      ? 'You already have active paid products. If needed, you can extend access or open another advanced scenario.'
      : 'You do not have active paid plans yet. You can still see the basic overview and early form signals.'}
    </AlertDescription>
   </Alert>
       ...
       >
        <CardHeader className="space-y-4">
         <div className="flex items-start justify-between gap-3">
          ...
          {isCurrentPlan ? <Badge>Active plan</Badge> : null}
         </div>
   ...
   <div className="text-sm text-muted-foreground">
    After payment, you will return to the app. If access does not update immediately, open{' '}
