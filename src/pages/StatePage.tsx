...
const buildCognitionMetrics = (report: PlayerStateReport): MetricRowProps[] => {
 ...
 const reaction = findBreakdown(report.zones.eyes.dataBreakdown, 'Go/No-Go', 'Reaction', 'reaction');
 if (!reaction) {
  const reactionHead = findBreakdown(bd, 'Reaction');
  if (reactionHead) {
   const val = extractNumeric(reactionHead);
   metrics.push({ label: 'Reaction', value: val !== null ? `${val}/100` : '—', trend: val !== null ? (val >= 75 ? 'up' : val >= 50 ? 'neutral' : 'down') : 'neutral' });
  }
 }
 const flex = findBreakdown(report.zones.eyes.dataBreakdown, 'Flexibility', 'flexibility');
 if (flex) {
  const val = extractNumeric(flex);
  metrics.push({ label: 'Cognitive flexibility', value: val !== null ? `${val}/100` : '—', trend: val !== null ? (val >= 75 ? 'up' : val >= 50 ? 'neutral' : 'down') : 'neutral' });
 }
 if (metrics.length === 0) {
  metrics.push({ label: 'Cognitive score', value: `${report.zones.head.score}/100` });
 }
 return metrics;
};
...
const buildPsychologyMetrics = (report: PlayerStateReport): MetricRowProps[] => {
 ...
 const metrics: MetricRowProps[] = [];
 const mood = findBreakdown(bd, 'Mood');
 if (mood) {
  const val = extractNumeric(mood);
  metrics.push({ label: 'Mood (14d avg)', value: val !== null ? `${val}/10` : '—', trend: val !== null ? (val >= 7 ? 'up' : val >= 5 ? 'neutral' : 'down') : 'neutral' });
 }
 const energy = findBreakdown(report.zones.back.dataBreakdown, 'Energy');
 if (energy) {
  const val = extractNumeric(energy);
  metrics.push({ label: 'Energy', value: val !== null ? `${val}/10` : '—', trend: val !== null ? (val >= 7 ? 'up' : val >= 5 ? 'neutral' : 'down') : 'neutral' });
 }
 const stress = findBreakdown(bd, 'Stress');
 if (stress) {
  const val = extractNumeric(stress);
  metrics.push({ label: 'Stress', value: val !== null ? `${val}/10` : '—', trend: val !== null ? (val <= 4 ? 'up' : val <= 6 ? 'neutral' : 'down') : 'neutral' });
 }
 const emotional = findBreakdown(bd, 'Emotions');
 if (emotional) {
  const val = extractNumeric(emotional);
  metrics.push({ label: 'Emotional balance (wheel)', value: val !== null ? `${val}/100` : '—' });
 }
 if (metrics.length === 0) {
  metrics.push({ label: 'Psychological score', value: `${report.zones.chest.score}/100` });
 }
 return metrics;
};
...
const buildLifestyleMetrics = (report: PlayerStateReport): MetricRowProps[] => {
 ...
 if (screenTime) {
  ...
  metrics.push({ label: 'Screen time', value: val !== null ? `${val} h/day` : '—', trend: val !== null ? (val <= 6 ? 'up' : val <= 9 ? 'neutral' : 'down') : 'neutral' });
 }
 const entertainment = findBreakdown(eyesBd, 'Entertainment');
 if (entertainment) {
  const val = extractNumeric(entertainment);
  metrics.push({ label: 'Entertainment', value: val !== null ? `${val} h/day` : '—' });
 }
 ...
 if (bwScores.length > 0) {
  ...
  metrics.push({ label: 'Balance wheel (avg)', value: `${avgBw}/100`, trend: avgBw >= 75 ? 'up' : avgBw >= 50 ? 'neutral' : 'down' });
 }
 if (metrics.length === 0) {
...
const buildGameMetrics = (report: PlayerStateReport): MetricRowProps[] => {
 ...
 const matches = findBreakdown(bd, 'Match');
 if (matches) {
  const val = extractNumeric(matches);
  metrics.push({ label: 'Matches (30d)', value: val !== null ? `${Math.round(val)}` : '—' });
 }
 if (metrics.length === 0 || (metrics.length === 0 && bd.some((item) => item.includes('missing')))) {
...
const StatePage = () => {
 ...
 const fetchReport = useCallback(async (forceRefresh = false) => {
  ...
  try {
   ...
   console.error('[StatePage] Error loading report:', err);
   setError('Failed to load state analysis. Please try again later.');
  } finally {
   setLoading(false);
  }
 }, []);
 ...
 return (
  <div className="container mx-auto px-4 py-6 max-w-6xl">
   <div className="mb-6">
    <PageIntro
     eyebrow="Current form index"
     title="State profile: one picture instead of scattered signals"
     description="This screen combines cognitive, psychological, lifestyle, and game signals into one overview, so you can see not only the final score but also the factor currently affecting your form the most."
     bullets={[
      "First, the overall picture by zones",
      "Then a breakdown by each factor",
      "The screen supports decisions instead of acting as a separate product",
     ]}
    />
   </div>
   ...
   {isEmpty && (
    <Card className="p-8 text-center">
     <div className="flex flex-col items-center gap-3 text-muted-foreground">
      ...
      <p className="text-base font-medium">Not enough data for analysis</p>
      <p className="text-sm max-w-md">
       Fill in the mood diary and complete tests to receive a personal state report.
      </p>
     </div>
    </Card>
   )}
    ...
    <Card className="p-6 border-destructive">
     <div className="flex items-start gap-3 text-destructive">
      ...
      <div className="space-y-2">
       <p className="font-medium">Analysis loading error</p>
        ...
        <>
         ...
         {report.fallbackUsed && (
          <div className="flex items-center gap-2 text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 px-3 py-2 rounded-md border border-amber-200 dark:border-amber-900">
           ...
           Analysis was completed without AI because the API is unavailable; the data was calculated algorithmically
          </div>
         )}
         ...
         <div className="text-xs text-muted-foreground pt-1 space-y-0.5 border-t">
          ...
          <p>
           <span className="font-medium">Model:</span> {report.model}
          </p>
         </div>
        </>
       )
   ...
   {(loading || hasData) && !error && (
    <div className="mt-8">
     <h2 className="text-base font-semibold mb-3">State factors</h2>
     {loading && !report ? (
