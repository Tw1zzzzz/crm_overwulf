...
const readPersistedGameStatsSnapshot = (): PersistedGameStatsSnapshot | null => {
 ...
 try {
  ...
  console.error('[GameStatsPage] Failed to restore table from localStorage:', error);
  return null;
 }
};
...
const clearPersistedGameStatsSnapshot = () => {
 ...
 try {
  ...
  console.error('[GameStatsPage] Failed to clear table in localStorage:', error);
 }
};
...
const writePersistedGameStatsSnapshot = (snapshot: PersistedGameStatsSnapshot) => {
 ...
 try {
  ...
  console.error('[GameStatsPage] Failed to save table in localStorage:', error);
 }
};
...
const GameStatsPage: React.FC = () => {
 ...
 const fetchGameStatsTemplate = async () => {
  ...
  try {
   ...
   if (!response.ok) {
    throw new Error(`Loading error таблицы (${response.status})`);
   }
   ...
   if (entries.length === 0) {
    ...
    toast.info('No game records for the selected period yet');
    return;
   }
 ...
 const handleGameStatsSubmit = async (data: GameStatsFormData) => {
  try {
   ...
   if (!response.ok) {
    ...
    throw new Error(errorData.message || 'Error while saving data');
   }

   await response.json();
   toast.success('Game metrics saved successfully');
   ...
   >
    <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
     ...
     <div className="grid grid-cols-2 gap-3 xl:min-w-[420px]">
      ...
      <div className="rounded-[22px] border p-4" style={{ backgroundColor: 'rgba(9, 14, 26, 0.34)', borderColor: 'rgba(148, 163, 184, 0.18)' }}>
       <div className="text-[11px] uppercase tracking-[0.22em]" style={{ color: 'rgba(191, 219, 254, 0.78)' }}>Period</div>
       <div className="mt-2 text-lg font-semibold" style={{ color: COLORS.textColor }}>
        {gameStatsDateFrom && gameStatsDateTo ? `${gameStatsDateFrom} - ${gameStatsDateTo}` : 'Not selected'}
       </div>
      </div>
      <div className="rounded-[22px] border p-4" style={{ backgroundColor: 'rgba(9, 14, 26, 0.34)', borderColor: 'rgba(148, 163, 184, 0.18)' }}>
       <div className="text-[11px] uppercase tracking-[0.22em]" style={{ color: 'rgba(191, 219, 254, 0.78)' }}>Columns</div>
       <div className="mt-2 text-lg font-semibold" style={{ color: COLORS.textColor }}>{gameStatsColumns.length || 0}</div>
      </div>
      <div className="rounded-[22px] border p-4" style={{ backgroundColor: 'rgba(9, 14, 26, 0.34)', borderColor: 'rgba(148, 163, 184, 0.18)' }}>
       <div className="text-[11px] uppercase tracking-[0.22em]" style={{ color: 'rgba(191, 219, 254, 0.78)' }}>Rows</div>
    ...
    <CardHeader className="pb-3">
     <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div className="space-y-2">
       ...
       <CardTitle style={{ color: COLORS.textColor }}>Game metrics summary table</CardTitle>
       <CardDescription style={{ color: COLORS.textColorSecondary }}>
        The interface was updated visually only: loading, modes, players, and the table template keep the same logic.
       </CardDescription>
      </div>
     </div>
    </CardHeader>
     ...
     >
      <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
       <div className="space-y-2">
        ...
        >
         ...
         <SelectContent style={{ backgroundColor: COLORS.cardBackground, borderColor: COLORS.borderColor, color: COLORS.textColor }}>
          <SelectItem value="individual">One player</SelectItem>
       ...
       <div className="space-y-2">
        ...
        >
         <SelectTrigger style={inputStyle}>
          <SelectValue placeholder={gameStatsMode === 'individual' ? 'Select player' : 'Only for player mode'} />
         </SelectTrigger>
         <SelectContent style={{ backgroundColor: COLORS.cardBackground, borderColor: COLORS.borderColor, color: COLORS.textColor }}>
          {players.map((player) => (
