...
const Dashboard = () => {
 ...
 // Statistics всех players (для персонала)
  ...
  const loadData = async () => {
   try {
    ...
    if (isStaff) {
     ...
     // Для players загружаем персональные данные of API
     try {
      ...
      console.log(`[Dashboard] Processed ${loadedMoodEntries.length} records о настроении of API`);
      ...
      console.log(`[Dashboard] Processed ${loadedTestEntries.length} records о тестах of API`);
      ...
      // Обрабатываем данные для графиков of локального хранилища
 ...
 const loadStaffData = async () => {
  try {
   ...
   // Загружаем данные о тестах players
 ...
 const processRecentStats = (entries: MoodEntry[]): RecentStats => {
  ...
  // Сортируем entries по дате (от самых новых к старым)
