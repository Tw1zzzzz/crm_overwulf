...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
   ...
   if (result.success) {
    ...
    if (result.user) {
     setAuthState({
      data: {
       ...result.user,
       _updateTimestamp: Date.now() // Для инвалидации кэша ofображений
      },
      loading: false,
      error: null
     });
    ...
    toast.error(result.error || 'Avatar update error');
   }
 ...
 // Инициалofация при монтировании
 useEffect(() => {
  initializeAuth();
 }, [initializeAuth]);

 // Обработка changes в localStorage (синхронofация между вкладками)
 useEffect(() => {
