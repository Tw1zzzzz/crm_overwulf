...
class ExcelExporter {
  ...
  const exportData: ExcelExportData = {
   ...
   sheets: [
    ...
    },
    {
     ...
     headers: ['Report type', 'Count', 'Percent', 'Average rating'],
     data: this.generateTypeStatistics(reports)
    },
    {
     name: 'Statistics by authors',
     headers: ['Author', 'Report count', 'Average rating', 'Latest report'],
 ...
 /**
  * Экспорт корреляционного analysis
  */
  ...
  const exportData: ExcelExportData = {
   ...
   sheets: [
    ...
    },
    {
     ...
     headers: ['Variable 1', 'Variable 2', 'Correlation coefficient', 'Significance', 'Interpretation'],
 ...
 async exportSentimentAnalysis(sentimentData: any): Promise<void> {
  const exportData: ExcelExportData = {
   ...
   sheets: [
    ...
    {
     name: 'Report details',
     headers: ['ID report', 'Title', 'Overall sentiment', 'Joy', 'Sadness', 'Anger', 'Fear', 'Confidence'],
 ...
 /**
  * Экспорт данных кластерofации players
  */
  ...
  const exportData: ExcelExportData = {
   ...
   sheets: [
    ...
    },
    {
     ...
     headers: ['Player', 'Cluster', 'Average activity', 'Average responsiveness', 'Recommendations'],
    ...
    },
    {
     ...
     headers: ['Cluster', 'Center - Activity', 'Center - Responsiveness', 'Within-cluster variance'],
 ...
 /**
  * Экспорт комплексного report аналитики
  */
 async exportComprehensiveAnalytics(analyticsData: any): Promise<void> {
  ...
  // Добавляем дополнительные листы of других analysisов
  if (analyticsData.sentiment) {
   exportData.sheets.push({
    name: 'Sentiment details',
    headers: ['Report', 'Sentiment', 'Dominant emotion', 'Level'],
  ...
  if (analyticsData.correlations) {
   exportData.sheets.push({
    ...
    headers: ['Variable 1', 'Variable 2', 'Coefficient', 'P-value'],
 ...
 private generateTypeStatistics(reports: any[]): any[][] {
  ...
  return Object.entries(typeStats).map(([type, stats]: [string, any]) => [
   ...
   stats.totalRating > 0 ? (stats.totalRating / stats.count).toFixed(1) : 'Not rated'
  ]);
 }
 ...
 private generateAuthorStatistics(reports: any[]): any[][] {
  ...
  return Object.entries(authorStats).map(([author, stats]: [string, any]) => [
   ...
   stats.count,
   stats.totalRating > 0 ? (stats.totalRating / stats.count).toFixed(1) : 'Not rated',
   stats.lastReport ? new Date(stats.lastReport).toLocaleDateString('en-US') : 'N/A'
  ]);
 }
