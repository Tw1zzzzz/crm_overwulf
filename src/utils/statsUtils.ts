...
/**
 * Sunпомогательные функции для работы с датами
 */
...
export const getMoodByDayOfWeek = (entries: MoodEntry[]): ChartDataPoint[] => {
 ...
 // Инициалofация массива с нулевыми значениями для каждого дня
...
export const prepareMoodDataByTimeRange = (
 ...
 // Инициалofируем данные для каждой даты
 const initialData = dates.map(date => ({
  date: format(date, dateFormat, { locale: enUS }),
 ...
 // Фильтруем entries для выбранного периода
 ...
 filteredEntries.forEach(entry => {
  try {
   ...
   const dateString = format(entryDate, dateFormat, { locale: enUS });
...
/**
 * Вычисляет агрегированную статистику для набора entries настроения
 */
