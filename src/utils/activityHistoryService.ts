...
/**
 * Интерфейс для entries активности
 */
...
const activityHistoryService = {
 ...
 getMonthlyActivity: async (): Promise<{
  ...
  try {
   ...
   // Получаем данные of ответа
   const activities = response.data.data.activities || [];
   console.log(`[History Service] Received ${activities.length} entries активности за месяц`);
   
   // Проверяем, есть ли entries о настроении (mood_track)
   const moodTrackActivities = activities.filter(a => a.action === 'mood_track');
   console.log(`[History Service] Found entries о настроении: ${moodTrackActivities.length}`);
   ...
   if (response.data.data.period) {
    ...
    // Если период not specified, используем текущий месяц
    const now = new Date();
    period = {
     start: new Date(now.getFullYear(), now.getMonth(), 1).toISOString(),
     end: new Date(now.getFullYear(), now.getMonth() + 1, 0).toISOString()
 ...
 /**
  * Formatирование времени в удобный формат
  */
 ...
 /**
  * Formatирование даты периода в человекочитаемый формат
  */
 ...
 /**
  * Formatирование периода в человекочитаемый формат
  */
