   ...
   return (
    <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg max-w-sm">
     ...
     <div className="space-y-1 text-sm">
      ...
      <p><strong>Responsiveness:</strong> {(player.avgResponsiveness * 100).toFixed(1)}%</p>
      <p><strong>Reportов выполнено:</strong> {player.stats.reportsCompleted}</p>
      <p><strong>Average rating:</strong> {player.stats.averageRating.toFixed(1)}</p>
     </div>
  ...
  <div className="space-y-6">
        ...
        {(selectedCluster === null ? clusters : [selectedClusterInfo]).filter(Boolean).map(cluster => (
         <Scatter
          ...
          data={[{
           ...
           playerName: `Centroid: ${cluster!.name}`,
           clusterName: 'Cluster center'
          }]}
          fill={cluster!.color}
          shape="diamond"
         />
   ...
   {viewMode === 'distribution' && (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
     ...
     {/* Wedавнение характеристик кластеров */}
     <Card>
      <CardHeader>
       <CardTitle>📈 Wedавнение центроидов</CardTitle>
      </CardHeader>
      <CardContent>
       <ResponsiveContainer width="100%" height={300}>
        <BarChart data={clusterComparison}>
         ...
         <Tooltip 
          formatter={(value, name) => [
           ...
           name === 'activity' ? 'Activity' : 'Responsiveness'
          ]}
         />
         <Bar dataKey="activity" name="Activity" fill="#3b82f6" />
         <Bar dataKey="responsiveness" name="Responsiveness" fill="#10b981" />
        </BarChart>
       </ResponsiveContainer>
      </CardContent>
     </Card>
      ...
      <Card key={cluster.id} className="border-l-4" style={{ borderLeftColor: cluster.color }}>
       <CardHeader>
        <CardTitle className="flex items-center gap-2 text-lg">
         ...
         />
         {cluster.name}
         <Badge variant="secondary">{cluster.playerCount} players</Badge>
        </CardTitle>
       </CardHeader>
       <CardContent className="space-y-4">
        ...
        <div>
         <h4 className="font-medium text-sm mb-2">Centroid:</h4>
         <div className="text-sm space-y-1">
          <p>Activity: {(cluster.centroid.activity * 100).toFixed(1)}%</p>
          <p>Responsiveness: {(cluster.centroid.responsiveness * 100).toFixed(1)}%</p>
    ...
    <Card>
     ...
     <CardContent>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
       <div>
        <h4 className="font-medium mb-2">Description and characteristics</h4>
