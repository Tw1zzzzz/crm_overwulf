...
export default function DailyQuestionnairePanel({
 ...
 title = "Daily questionnaire without the heavy form feeling",
 description = "Input is split into clear blocks: sleep, screen time, and a final total check. It is faster to fill in and easier to review.",
 onSaved
 ...
 const qDateLabel = useMemo(() => {
  ...
  return Number.isNaN(parsed.getTime())
   ? qDate
   : parsed.toLocaleDateString("en-US", { day: "2-digit", month: "long", year: "numeric" });
 }, [qDate]);
 ...
 const handleSubmit = async () => {
  try {
   ...
   const message = error?.response?.data?.message || error?.message || "Save error";
   ...
   <section className="grid gap-5 xl:grid-cols-[1.02fr_0.98fr]">
    <div className="space-y-5">
     <Card style={{ backgroundColor: COLORS.cardBackground, borderColor: COLORS.borderColor, boxShadow: "0 1px 20px 0 rgba(0,0,0,.1)" }}>
      <CardHeader className="pb-2">
       <div className="flex items-start gap-3">
        ...
        <div>
         <CardTitle style={{ color: COLORS.textColor }}>Sleep и восстановление</CardTitle>
         <CardDescription style={{ color: COLORS.textColorSecondary }}>
          Fill in the sleep range or adjust hours manually.
         </CardDescription>
        </div>
       </div>
      </CardHeader>
      <CardContent className="space-y-4">
       ...
       <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="space-y-2">
         <Label style={{ color: COLORS.textColor }}>Sleep: с</Label>
        ...
        <div className="space-y-2">
         <Label style={{ color: COLORS.textColor }}>Sleep: до</Label>
       ...
       <div className="rounded-[22px] border p-4" style={{ borderColor: COLORS.borderColor, backgroundColor: "rgba(255,255,255,0.03)" }}>
        <Label style={{ color: COLORS.textColor }}>Sleep (часы, авто или вручную)</Label>
        <Input
         value={qSleep}
        ...
        <p className="mt-3 text-sm leading-6" style={{ color: COLORS.textColorSecondary }}>
         If you entered sleep time `с` и `до`, система уже подсказывает продолжительность. Поле можно оставить как есть или скорректировать вручную.
        </p>
       </div>
      </CardContent>
     </Card>
     ...
     <Card style={{ backgroundColor: COLORS.cardBackground, borderColor: COLORS.borderColor, boxShadow: "0 1px 20px 0 rgba(0,0,0,.1)" }}>
      <CardHeader className="pb-2">
       <div className="flex items-start gap-3">
        ...
        <div>
         ...
         <CardDescription style={{ color: COLORS.textColorSecondary }}>
          Total time and a clean category breakdown.
         </CardDescription>
        </div>
       </div>
      </CardHeader>
      <CardContent className="space-y-4">
       <div className="space-y-2">
        <Label style={{ color: COLORS.textColor }}>Total screen time</Label>
        <Input
         value={qScreen}
         onChange={(event) => setQScreen(event.target.value)}
         inputMode="decimal"
         placeholder="You can leave this empty if the breakdown is filled in"
       ...
       <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <Input value={qScreenCommunication} onChange={(event) => setQScreenCommunication(event.target.value)} inputMode="decimal" placeholder="Communication" className="rounded-2xl" style={fieldStyle} />
        <Input value={qScreenBrowser} onChange={(event) => setQScreenBrowser(event.target.value)} inputMode="decimal" placeholder="Browser" className="rounded-2xl" style={fieldStyle} />
        <Input value={qScreenStudy} onChange={(event) => setQScreenStudy(event.target.value)} inputMode="decimal" placeholder="Study / work" className="rounded-2xl" style={fieldStyle} />
       </div>
      </CardContent>
     </Card>
    </div>
    ...
    <Card style={{ backgroundColor: COLORS.cardBackground, borderColor: COLORS.borderColor, boxShadow: "0 1px 20px 0 rgba(0,0,0,.1)" }}>
     <CardHeader className="pb-2">
      <div className="flex items-start gap-3">
       ...
       <div>
        <CardTitle style={{ color: COLORS.textColor }}>Completion control</CardTitle>
        <CardDescription style={{ color: COLORS.textColorSecondary }}>
         Before saving, you can immediately see category totals and possible errors.
        </CardDescription>
       </div>
      </div>
     </CardHeader>
      ...
      <div className="rounded-[22px] border p-4" style={{ borderColor: COLORS.borderColor, backgroundColor: "rgba(255,255,255,0.03)" }}>
       ...
       <div className="mt-2 text-sm leading-6" style={{ color: COLORS.textColorSecondary }}>
        {hasQuestionnaireTotal
         ? `Total time задано: ${questionnaireTotalScreen.toFixed(1)} ч`
         : "You do not need to set total time if the category breakdown is filled in."}
       </div>
      </div>
      ...
      >
       <div className="text-sm font-medium" style={{ color: COLORS.textColor }}>
        {questionnaireBreakdownSum === 0
         ? "Fill in at least one category or total screen time"
         : isQuestionnaireExceeded
          ? "Category total exceeds total screen time"
          : "Data looks consistent"}
       </div>
       <p className="mt-2 text-sm leading-6" style={{ color: COLORS.textColorSecondary }}>
        {questionnaireBreakdownSum === 0
         ? "As soon as data appears, this block will show whether everything lines up logically."
         : hasQuestionnaireTotal
          ? `${questionnaireBreakdownSum.toFixed(1)} h of ${questionnaireTotalScreen.toFixed(1)} ч общего времени`
          : "The system will use the category total as total screen time."}
       </p>
      </div>
