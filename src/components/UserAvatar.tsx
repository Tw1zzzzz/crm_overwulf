...
const UserAvatar: React.FC<UserAvatarProps> = ({ 
 ...
 // Локальное состояние для отслеживания загрузки images
 ...
 useEffect(() => {
  if (imageError && retryCount < 2) {
   const timer = setTimeout(() => {
    console.log(`🔄 Retrying avatar upload (${retryCount + 1}/2)`);
 ...
 // Если пользователь not specified, показываем пустой аватар
 if (!user) {
  return (
   <Avatar 
 ...
 // Обработчик успешной загрузки images
 const handleImageLoad = () => {
  setImageLoaded(true);
  setImageError(false);
 };

 // Обработчик ошибки загрузки images
