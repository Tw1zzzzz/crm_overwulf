...
// Typeы данных для API аналитики
...
// API для работы с Колесом Balance
...
export const getPlayerBalanceWheels = async (playerId: string | any) => {
 ...
 try {
  console.log(`[API] Balance wheel data request for player: ${actualPlayerId}`);
  ...
  console.log(`[API] Received balance wheel data (${normalized.data.length} entries)`);
  return normalized;
 } catch (error) {
  console.error(`[API] Error while getting data колеса баланса:`, error);
...
// API для получения данных о настроении и энергии players с фильтрацией по дате
...
// API для получения всех данных о настроении players с фильтрацией по дате
...
export const updatePrivilegeKey = async (privilegeKey: string) => {
 try {
  ...
  throw new Error(error.response?.data?.message || 'Error while updating privilege key');
 }
};
...
// API функции для работы с reportsи team
...
// API функции для корреляционного analysis

/**
 * Получить корреляции между reportsи team и настроением players
 */
...
/**
 * Получить корреляции между reportsи и колесом баланса
 */
...
/**
 * Получить комплексный корреляционный analysis
 */
...
/**
 * Получить статистику корреляционного analysis
 */
...
/**
 * Получить analysis сентимента team reports
 */
...
/**
 * Получить кластерный analysis players
 */
...
/**
 * Получить analysis временных рядов
 */
