...
abstract class DataRepository<T> {
 ...
 /**
  * Получает элементы of localStorage
  */
 ...
 /**
  * Получает все элементы of хранилища
  */
 public getAll(): T[] {
 ...
 /**
  * Удаляет элемент of хранилища и добавляет операцию в очередь sync
  */
 public delete(id: string): void {
  // Проверка на валидность ID
  if (!id || id === 'undefined' || id === 'null') {
   console.error(`[DataRepository] Attempted to delete item with invalid ID: ${id}`);
   return;
  }
  ...
  if (itemToDelete) {
   ...
   console.warn(`[DataRepository] Element with ID ${id} не найден для удаления`);
  }
 }
 ...
 /**
  * Очищает все данные of хранилища
  */
 public clear(): void {
  this.saveToStorage([]);
 }
...
export class MoodRepository extends DataRepository<MoodEntry> {
 ...
 /**
  * Преобразует даты of строк в объекты Date при получении of хранилища
  */
